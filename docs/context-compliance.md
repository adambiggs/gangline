...
| `compact_request`, `requester`, `issue_tokens`, `first_threshold` | Who asked Gangline to submit compaction and the context `A` and first rung `F` at issue time. A request is not proof that the harness executed it. |
...
| `evidence_availability`, `evidence_result`, `issue_tokens`, `first_threshold` | Whether patrol's independent drop proof was available. A known issue context `A` makes it available at every band, and a seen marker or proved drop establishes itself even where `A` was not known. `A` and the first rung `F` stay recorded to interpret rows written before patrol polled unconditionally, when a low first band could put the proof out of reach. That CTD is kept distinct from available evidence that did not establish cause. |
