...
| **vet check** | at every checkpoint a veterinarian examines each dog before it runs on | `gang vet` — the strategy-rot check: each profile's scraped markers against the installed harness (alias: `doctor`) |
...
| **hook set** | the snow hook dug in; the team stands anchored until the musher pulls it | gated — a modal owns the input box, so nothing can land in it; sends are refused rather than pasted into a dialog where a keystroke would answer it |
