...
| `collar_input target` | print human-authored composer contents; 1 when the harness has drawn no composer, 3 when the pane itself could not be read, and a collar may declare further statuses only when Gangline has learned their meaning. `claude-code` declares 4: a composer is drawn, but it belongs to a selected in-process subagent, so keys typed there reach the child. Gangline carries 4 through as its own refusal rather than flattening it into absence. Any status Gangline does not recognise is a loud unknown that preserves the raw value, never absence |
