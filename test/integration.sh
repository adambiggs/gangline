...
test_now_ns() { "$ROOT/libexec/gang-clock" now; }
...
# The floor above is a race and a starved box can lose it. Turn that setup
# failure into a named result so the remaining checks and summary still run.
#
...
# A run that ends early still owes a reading. One retry handles an isolated
# starved submission; it cannot rescue a box starving nearly all of them. So
...
submitted() { # $1 description, $2 agent
  equal "$1" "" "$("$GANG" capture --composer "$2")"
}
...
# give the child a writable stand-in for an operator home.
...
# grows faster than that file does. Sourcing parts keeps the suite one program
# while letting lint analyze each part inside the gate's memory budget.
#
...
# Join the isolated self-test here so its output stays contiguous and its checks
# and failures remain part of this suite's one summary.
lanes_rc=0
