...
# A SWEEP AND A MINT SHARE ONE ORDERING QUESTION. The mint reserves a directory
# before publishing its final identity; a sweep that reads the window register
# inside that gap must not decide the new reservation is an orphan. The shim
# stops only the final @gl_spool publication, after recording its token. The
# tick therefore runs at the exact old race seam, with no clock or polling:
# the provisional claim must already protect the reserved directory.
mint_sweep_bin="$RUN_ROOT/mint-sweep-bin"
mint_sweep_token_file="$RUN_ROOT/mint-sweep-token"
mint_sweep_out="$RUN_ROOT/mint-sweep.out"
mint_sweep_err="$RUN_ROOT/mint-sweep.err"
mint_sweep_rc_file="$RUN_ROOT/mint-sweep.rc"
mint_sweep_ready="mint-sweep-ready-$$"
mint_sweep_release="mint-sweep-release-$$"
mkdir -p "$mint_sweep_bin"
cat > "$mint_sweep_bin/tmux" <<SH
#!/bin/sh
REAL='$REAL_TMUX'
. "\$GANG_TEST_PATH_SHIM_GUARD"
...
next_is_token=0
for argument do
  if [ "\$next_is_token" -eq 1 ]; then final_token="\$argument"; break; fi
  [ "\$argument" != @gl_spool ] || next_is_token=1
done
if [ -n "\$final_token" ]; then
  printf '%s\n' "\$final_token" > "\$GANG_MINT_SWEEP_TOKEN_FILE" || exit \$?
  "\$REAL" wait-for -S "\$GANG_MINT_SWEEP_READY" || exit \$?
  "\$REAL" wait-for -L "\$GANG_MINT_SWEEP_RELEASE" || exit \$?
  "\$REAL" wait-for -U "\$GANG_MINT_SWEEP_RELEASE" || exit \$?
fi
exec "\$REAL" "\$@"
SH
chmod +x "$mint_sweep_bin/tmux"
tmux wait-for -L "$mint_sweep_release"
tmux wait-for "$mint_sweep_ready" &
mint_sweep_ready_waiter=$!
(
  mint_sweep_rc=0
  PATH="$mint_sweep_bin:$PATH" \
    GANG_MINT_SWEEP_TOKEN_FILE="$mint_sweep_token_file" \
    GANG_MINT_SWEEP_READY="$mint_sweep_ready" \
    GANG_MINT_SWEEP_RELEASE="$mint_sweep_release" \
    "$HITCH" mint-sweep -c spoolable -d /tmp \
    >"$mint_sweep_out" 2>"$mint_sweep_err" || mint_sweep_rc=$?
  printf '%s\n' "$mint_sweep_rc" > "$mint_sweep_rc_file"
) &
mint_sweep_pid=$!
wait "$mint_sweep_ready_waiter"
mint_sweep_token="$(cat "$mint_sweep_token_file")"
mint_sweep_spool="$GANG_LOCK_DIR/spool/$mint_sweep_token"
equal "a mint publishes a pending claim before its final spool identity" \
  "$mint_sweep_token" \
  "$(tmux show-options -wqv -t "$(window_id mint-sweep)" @gl_spool_pending)"
[ -d "$mint_sweep_spool" ] \
  && pass "the mint reserves its spool before final publication" \
  || fail "the mint reserves its spool before final publication" \
    "$mint_sweep_spool is absent at the final-publication seam"
"$GANG" tick >/dev/null
[ -d "$mint_sweep_spool" ] \
  && pass "a concurrent orphan sweep preserves the minting window's reservation" \
  || fail "a concurrent orphan sweep preserves the minting window's reservation" \
    "$mint_sweep_spool was swept before its final identity could be published"
tmux wait-for -U "$mint_sweep_release"
wait "$mint_sweep_pid"
equal "the mint completes after the sweep releases final publication" 0 \
  "$(cat "$mint_sweep_rc_file")"
equal "the completed mint clears its provisional claim" "" \
  "$(tmux show-options -wqv -t "$(window_id mint-sweep)" @gl_spool_pending)"
equal "the completed mint publishes the reservation it protected" \
  "$mint_sweep_token" \
  "$(tmux show-options -wqv -t "$(window_id mint-sweep)" @gl_spool)"
"$GANG" drop mint-sweep >/dev/null
...
contains "mail distinguishes kept ambiguity from mail awaiting delivery" \
  "$held_live_mail" \
  "0 waiting for delivery, 0 deferred until an ordinary delivery or its deadline, 1 kept (not awaiting delivery)"
