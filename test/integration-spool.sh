...
if nohook_out="$(printf 'MARK_NOHOOK' |
  "$GANG" send nodrain --from tester 2>&1)"; then
...
if super_out="$(printf 'MARK_LONE_SUPERSEDE' |
  "$GANG" send alpha --from tester --supersede --live-only 2>&1)"; then
...
# a sender their message had failed when it had been accepted. The wording must
# describe the composed outcome, not each intermediate clause.
...
if live_only_out="$(printf 'MARK_LIVE_ONLY' |
  "$GANG" send parker --from tester --live-only 2>&1)"; then
...
spool_noop_out="$(printf 'MARK_ANNOUNCED' |
  "$GANG" send parker --from tester --spool --supersede 2>&1 || true)"
...
printf 'MARK_OTHER_SENDER' |
  "$GANG" send parker --from other >/dev/null
printf 'MARK_STALE' | "$GANG" send parker --from tester >/dev/null
contains "a second message from one sender does not replace the first" \
  "$("$GANG" status parker)" "spooled: 3"
printf 'MARK_LATEST' |
  "$GANG" send parker --from tester --supersede >/dev/null
...
excludes "nor the refused body" "$parker_drained" "MARK_ANNOUNCED"
...
printf 'MARK_LIVE_PREDECESSOR' |
  "$GANG" send parker --from tester >/dev/null
tmux send-keys -t "$parker_id" C-u
...
printf 'MARK_BUNDLE_THREE' |
  "$GANG" send parker --from third >/dev/null
tmux send-keys -t "$parker_id" C-u
: > "$RUN_ROOT/claim-watch"
tmux wait-for "gang-spool-drain-$parker_id" &
bundle_drain_waiter=$!
...
# A STALL HERE MUST NAME MORE THAN A CHANNEL. Two rendezvous barriers can expire
# together, so the fixture records each step it
...
printf 'v2\tgangline\t-\tcontrol\tabadcafeabadcafe\n%s\n%s\n' MARK_INTERRUPTED \
...
printf 'v2\tother\tdeadbeef\trequest\tdeadbeefdeadbeef\n%s\n%s\n' MARK_ARCHIVED_HELD \
...
printf 'MARK_SECOND_ARCHIVE' |
  "$GANG" send archive-second --from tester >/dev/null
...
# every entry as a dot-file and commits it by rename. A write that dies between
# those steps leaves a fragment teardown must archive rather than erase.
...
printf 'v2\ttester\tdeadbeef\trequest\tdeadbeefdeadbeef\nhalf a message\n[gang:tester#deadbeef] MARK_FRAGMENT' \
...
printf 'MARK_UNACCOUNTABLE_WAITING' |
  "$GANG" send unaccountable --from tester >/dev/null
...
# ACCEPTED IS WHAT LICENSES A RETIREMENT. A superseding write must commit before
# the sender's waiting entries are removed. The stamp injects a failure before
# any replacement entry exists.
...
printf 'MARK_PREDECESSOR' |
  "$GANG" send superseder --from tester >/dev/null
...
printf 'MARK_UNSTAMPED_REPLACEMENT' |
  PATH="$RUN_ROOT/nostamp:$PATH" "$GANG" send superseder --from tester \
    --supersede > "$RUN_ROOT/nostamp.out" 2> "$RUN_ROOT/nostamp.err" \
  || nostamp_rc=$?
...
if halfway_out="$(printf 'MARK_COMMITTED_REPLACEMENT' |
  "$GANG" send superseder --from tester --supersede 2>&1)"; then
...
if printf 'MARK_TRAILING_REPLACEMENT' |
  "$GANG" send superseder --from tester --supersede >/dev/null 2>&1; then
...
if nocommit_out="$(printf 'MARK_UNCOMMITTED_REPLACEMENT' |
  PATH="$RUN_ROOT/nocommit:$PATH" "$GANG" send superseder --from tester \
    --supersede 2>&1)"; then
...
if norollback_out="$(printf 'MARK_UNDELIVERED' |
  PATH="$RUN_ROOT/norollback:$PATH" "$GANG" send norollback --from tester \
    --supersede 2>&1)"; then
...
printf 'MARK_TRAVERSAL' |
  "$GANG" send traversal --from tester >/dev/null
...
printf 'MARK_MAIL_ONE' |
  "$GANG" send mailer --from tester >/dev/null
printf 'MARK_MAIL_TWO' |
  "$GANG" send mailer --from other >/dev/null
...
mail_out="$("$GANG" queue mailer)"
contains "queue prints the first waiting body" "$mail_out" "MARK_MAIL_ONE"
contains "queue prints the second waiting body" "$mail_out" "MARK_MAIL_TWO"
contains "queue prints the third waiting body" "$mail_out" "MARK_MAIL_THREE"
contains "queue names the first sender" "$mail_out" "from tester"
contains "queue names the other sender" "$mail_out" "from other"
mail_order="$(printf '%s\n' "$mail_out" |
  grep -oE 'MARK_MAIL_ONE|MARK_MAIL_TWO|MARK_MAIL_THREE' |
  awk '!seen[$0]++' | tr '\n' ' ')"
equal "queue prints waiting bodies in stamp order" \
  "MARK_MAIL_ONE MARK_MAIL_TWO MARK_MAIL_THREE " "$mail_order"
mail_status_after="$("$GANG" status mailer)"
contains "queue leaves the waiting count unchanged before its read" \
  "$mail_status_before" "spooled: 3"
contains "queue leaves the waiting count unchanged after its read" \
  "$mail_status_after" "spooled: 3"
equal "queue leaves every entry filename untouched" "$mail_names_before" \
  "$(cd "$mailer_spool" && ls)"

tmux set-option -w -t "$mailer_id" @gl_collar no-such-collar
mail_without_collar="$("$GANG" queue mailer)"
contains "queue reads bodies without loading the target collar" \
...
printf 'v2\tother\tfacefeed\trequest\tfacefeedfacefeed\n%s\n%s\n' MARK_MAIL_HELD \
  '[gang:other#facefeed] MARK_MAIL_HELD [/gang:other#facefeed]' \
  > "$mailer_failed"
mail_with_held="$("$GANG" queue mailer)"
contains "queue prints a held body" "$mail_with_held" "MARK_MAIL_HELD"
contains "queue labels held delivery as unverified" \
...
mail_reference="$("$GANG" queue mailer)"
mailer_self_rc=0
TMUX_PANE="$mailer_self_pane" "$GANG" queue > "$RUN_ROOT/mail-self.out" 2>&1 \
  || mailer_self_rc=$?
mailer_self_out="$(grep -v '^gang: WARNING: executing dirty ' \
  "$RUN_ROOT/mail-self.out" || true)"
equal "bare queue reads the calling agent's own queue" \
...
mail_after_self="$("$GANG" queue mailer)"
...
printf 'MARK_MAIL_LATER' |
  "$GANG" send mailer --from tester >/dev/null
mail_later_rc=0
TMUX_PANE="$mailer_self_pane" "$GANG" queue > "$RUN_ROOT/mail-later.out" 2>&1 \
  || mail_later_rc=$?
...
mail_malformed_rc=0
TMUX_PANE="$mailer_self_pane" "$GANG" queue \
...
[ "$mail_malformed_rc" -ne 0 ] \
  ...
  || fail "a bodyless spool entry refuses a self-read" \
    "queue unexpectedly returned zero"
...
GANG_ARCHIVE_DIR="$mail_archive_blocker" TMUX_PANE="$mailer_self_pane" \
  "$GANG" queue >"$RUN_ROOT/mail-archive-refusal.out" 2>&1 \
...
mail_archive_recovery="$(TMUX_PANE="$mailer_self_pane" "$GANG" queue 2>&1)"
contains "the untouched message remains readable after archive repair" \
  "$mail_archive_recovery" "MARK_MAIL_ARCHIVE_REFUSAL"

# A SHELL FILTER MAY HIDE OUTPUT, BUT IT CANNOT DESTROY THE ONLY COPY. Here tail
# consumes all of gang queue's stdout and prints only its end. The body prefix
# is absent from the filtered view, while the
...
excludes "tail hides the head of a long self-read" \
...
mail_foreign_out="$(TMUX_PANE="$alpha_tmux_pane" "$GANG" queue mailer)"
...
equal "and an operator outside the team consumes nothing either" \
  "$mail_foreign_before" \
  "$("$GANG" queue mailer >/dev/null; cd "$mailer_spool" && ls)"
...
printf 'MARK_SELF_READ_LOCK' |
  "$GANG" send mail-lock --from tester >/dev/null
mail_lock_entry=""
...
TMUX_PANE="$mail_lock_pane" GANG_MAIL_LOCK_ENTRY="$mail_lock_entry" \
  ...
  GANG_MAIL_LOCK_RELEASE="$mail_lock_release" "$GANG" queue \
...
printf 'MARK_STALE_REAP_EXCLUSIVE' |
  "$GANG" send stale-reap --from tester >/dev/null
stale_reap_entry=""
...
stale_reap_identity="$("$ROOT/libexec/gang-process-identity" --tick "$$" "$GANG_SESSION")"
IFS=$'\t' read -r _ _ _ _ _ _ _ stale_reap_namespace <<<"$stale_reap_identity"
stale_reap_dead_owner="v1:99999999:0:$stale_reap_namespace"
ln -s "$stale_reap_dead_owner" "$stale_reap_lock"
...
equal "the guarded reclaimer has not replaced the dead owner yet" \
  "$stale_reap_dead_owner" "$(readlink "$stale_reap_lock")"
TMUX_PANE="$stale_reap_pane" \
  ...
  stale-reap-mail "$GANG" queue >"$RUN_ROOT/stale-reap-a.out" 2>"$RUN_ROOT/stale-reap-a.err" &
stale_reap_a_pid=$!
...
stale_reap_mail="$(TMUX_PANE="$stale_reap_pane" "$GANG" queue)"
...
TMUX_PANE="$stale_reap_pane" \
  ...
  "$GANG" queue >"$RUN_ROOT/stale-reap-c.out" 2>"$RUN_ROOT/stale-reap-c.err" &
stale_reap_c_pid=$!
...
TMUX_PANE="$stale_reap_pane" \
  ...
  GANG_STALE_REAP_A_RELEASE="$stale_reap_a_release" \
  sh -c 'rc=0; "$@" || rc=$?; printf "exit:%s\\n" "$rc" > "$GANG_STALE_REAP_A_EVENT"; exit "$rc"' \
  stale-reap-reader "$GANG" queue >"$RUN_ROOT/stale-reap-reader.out" 2>"$RUN_ROOT/stale-reap-reader.err" &
...
printf 'MARK_STALE_REAP_RELEASED' |
  ...
  GANG_STALE_REAP_C_READY="$stale_reap_c_ready" \
  GANG_STALE_REAP_C_RELEASE="$stale_reap_c_release" \
  "$GANG" send stale-reap --from tester \
...
stale_reap_exiting_identity="$("$ROOT/libexec/gang-process-identity" \
  --tick "$stale_reap_exiting_pid" "$GANG_SESSION")"
IFS=$'\t' read -r _ stale_reap_exiting_token _ _ _ _ _ stale_reap_exiting_namespace \
  <<<"$stale_reap_exiting_identity"
stale_reap_exiting_owner="v1:$stale_reap_exiting_pid:$stale_reap_exiting_token:$stale_reap_exiting_namespace"
ln -s "$stale_reap_exiting_owner" "$stale_reap_lock"
...
printf 'v2\ttester\tbadc0ffe\trequest\tbadc0ffebadc0ffe\n%s\n%s\n' \
  vanished-claim MARK_VANISHED_CLAIM > "$vanished_claim"
...
empty_mail_out="$("$GANG" queue empty-mailbox)"
contains "queue exits cleanly on an empty queue" \
...
printf 'v2\theld-sender\t%s\trequest\tdeadbeefdeadbeef\n%s\n%s\n' \
  ...
  > "$held_entry"
held_live_mail="$("$GANG" queue held-target)"
contains "an unverified submission from a live stable sender remains readable" \
  "$held_live_mail" "MARK_STABLE_HELD"
contains "queue distinguishes kept ambiguity from mail awaiting delivery" \
  "$held_live_mail" \
  "0 waiting for delivery, 1 kept (not awaiting delivery)"
...
held_retired_mail="$("$GANG" queue held-target 2>"$RUN_ROOT/held-retired-mail.err")"
excludes "queue retires an unverified record whose stable sender is gone" \
...
excludes "a fresh agent reusing the sender name does not revive the old record" \
  "$("$GANG" queue held-target)" "MARK_STABLE_HELD"
...
held_interrupted_mail="$("$GANG" queue held-target)"
contains "queue names a live sender's interrupted delivery outcome" \
  "$held_interrupted_mail" "delivery worker ended after claiming this body"
contains "queue keeps the interrupted body readable" \
...
"$GANG" drop held-sender >/dev/null
held_interrupted_retired="$("$GANG" queue held-target 2>"$RUN_ROOT/held-interrupted-retired.err")"
excludes "queue retires an interrupted record whose stable sender is gone" \
...
printf 'v2\theld-sender\t%s\trequest\tbaddecafbaddecaf\n%s\n%s\n' \
  ...
  > "$held_self_retired"
TMUX_PANE="$held_target_pane" "$GANG" queue \
  >"$RUN_ROOT/held-self-mail.out" 2>"$RUN_ROOT/held-self-mail.err"
contains "self-queue consumes the waiting body" \
  "$(<"$RUN_ROOT/held-self-mail.out")" "MARK_SELF_WAITING"
excludes "self-queue retires gone-sender ambiguity instead of printing it" \
...
[ -n "$held_self_read_archive" ] && [ -n "$held_self_retire_archive" ] \
  ...
  && pass "self-queue gives consumed mail and retired ambiguity separate archives" \
  || fail "self-queue gives consumed mail and retired ambiguity separate archives" \
    "read=${held_self_read_archive:-missing} retired=${held_self_retire_archive:-missing}"
contains "self-queue reports the distinct retirement destination" \
...
excludes "queue cannot revive the record roster retired" \
  "$("$GANG" queue held-target)" "MARK_ROSTER_RETIRES"
...
printf 'v2\ttester\tabcd1234\trequest\tabcd1234abcd1234\n%s\n%s\n' MARK_PORCELAIN_QUEUE \
...
# the operator sentinel and no parent name.
...
printf 'v2\ttester\taaaabbbb\trequest\taaaabbbbaaaabbbb\n%s\n%s\n' MARK_AGE_ONE_HOUR \
...
printf 'v2\tother\tccccdddd\trequest\tccccddddccccdddd\n%s\n%s\n' MARK_AGE_TWO_HOURS \
...
printf 'v2\ttester\teeeeffff\trequest\teeeeffffeeeeffff\n%s\n%s\n' MARK_BAD_STAMP \
...
printf 'MARK_PARKED_ONE' |
  "$GANG" send preempt --from tester >/dev/null
printf 'MARK_PARKED_TWO' |
  "$GANG" send preempt --from other >/dev/null
printf 'MARK_PARKED_THREE' |
  "$GANG" send preempt --from third >/dev/null
...
# leaves for the next one. The delivery has to be witnessed on the pane rather
# than in gang's
...
if identityless_out="$(printf 'MARK_NO_IDENTITY' |
  "$GANG" send identityless --from tester 2>&1)"; then
...
# later world with it.
printf 'MARK_ADOPTED' |
  "$GANG" send taken --from tester >/dev/null 2>&1 || true
...
if unverified_out="$(printf 'MARK_UNVERIFIED' |
  "$GANG" send unverified --from tester 2>&1)"; then
...
printf 'MARK_WEDGED' | "$GANG" send wedged --from tester >/dev/null
printf 'MARK_HELD_TWO' | "$GANG" send wedged --from other >/dev/null
printf 'MARK_HELD_THREE' | "$GANG" send wedged --from third >/dev/null
...
if hard_supersede_out="$(printf 'MARK_HARD_REPLACEMENT' |
  "$GANG" send wedged --from tester --supersede 2>&1)"; then
...
claimlost_out="$(printf 'MARK_CLAIM_LOST' |
  "$GANG" send claimlost --from tester)"
...
# claimed envelope in the private composer's box, with no staged option. It
# remains delivery state rather than a human draft.
...
equal "the private composer reader preserves the claimed envelope bytes" \
  "$claimlost_body" "$("$GANG" capture --composer claimlost)"
"$GANG" tick >/dev/null
...
excludes "the old composer body is withdrawn before another delivery" \
  "$("$GANG" capture --composer claimlost)" "MARK_CLAIM_LOST"
...
claimrace_out="$(printf 'MARK_CLAIM_RACE' |
  "$GANG" send claimrace --from tester)"
...
claimrace_tick_out="$(PATH="$claimrace_bin:$PATH" \
  ...
  "$GANG" send claimrace --fire "${claimrace_timed##*/}" \
...
printf 'v2\tgangline\t-\tcontrol\t1a2b3c4d1a2b3c4d\n%s\n%s\n' \
  large-recovery "$claimlarge_body" > "$claimlarge_claim"
"$GANG" tick >/dev/null
...
excludes "a large exact recovery body is cleared without a tmux option limit" \
  "$("$GANG" capture --composer claimlarge)" "MARK_LARGE_RECOVERY_"
...
claimlarge_followup="$(printf 'MARK_LARGE_RECOVERED' |
  "$GANG" send claimlarge --from tester)"
...
printf 'v2\tgangline\t-\tcontrol\t5e6f78905e6f7890\n%s\n%s\n' \
  existing-stage "$claimexisting_body" > "$claimexisting_claim"
...
equal "an interrupted recovery leaves the existing staged composer untouched" \
  "$claimexisting_body" "$("$GANG" capture --composer claimexisting)"
...
printf 'MARK_CARRIED' |
  "$GANG" send carried --from tester >/dev/null 2>&1 || true
"$GANG" adopt carried -c spoolable-adopt >/dev/null
...
# saw, queue never printed, and only the teardown archive ever met again.
...
contains "and queue hands over the body it is holding" \
  "$("$GANG" queue stranded)" "MARK_STRANDED_FRAGMENT"
...
excludes "a fragment whose sender is still running is not reported" \
  "$("$GANG" queue stranded)" "MARK_INFLIGHT_FRAGMENT"
...
lost_out="$(printf 'MARK_LOSTSCREEN' |
  "$GANG" send lostscreen --from tester 2>&1)" || lost_rc=$?
rm -f "$RUN_ROOT/lost-seen" "$RUN_ROOT/lost-armed"
...
printf 'MARK_LOSTDRAIN' |
  "$GANG" send lostscreen --from tester >/dev/null
...
excludes "and never leaves it where a later boundary would send it again" \
  "$lost_status" "spooled:"
contains "queue warns its reader they may have seen this already" \
  "$("$GANG" queue lostscreen)" "you may have seen this already"
contains "and still hands over the body" \
  "$("$GANG" queue lostscreen)" "MARK_LOSTDRAIN"
...
at_out="$(printf 'MARK_TIMED_BODY' | PATH="$at_bin:$PATH" \
  "$GANG" send timed --at 90m --from tester)"
...
at_fire_name="$(tail -n 3 "$at_args" | head -n 1)"
...
# sequence instead. It is seven entries: gang, send, <name>, --fire, <entry>,
# --unit, <unit>.
equal "the timer's callback is gang's own send --fire" "send timed --fire" \
  "$(tail -n 6 "$at_args" | head -n 3 | tr '\n' ' ' | sed 's/ $//')"
...
PATH="$at_bin:$PATH" "$GANG" send timed --fire "${at_entry##*/}" \
...
GANG_TEST_NOW="$(( $(date +%s) + 7200 ))" PATH="$at_bin:$PATH" \
  "$GANG" send timed --fire "${at_entry##*/}" --unit "$at_unit" >/dev/null 2>&1
if [ -f "$at_entry" ]; then
...
contains "and the promoted entry is the envelope minted at park time" \
  "$("$GANG" queue timed)" "[gang:self-declared:tester#"
...
printf 'MARK_TIMED_CLEAR' | PATH="$at_bin:$PATH" \
  "$GANG" send timed-clear --at 2h --from tester >/dev/null
at_clear_unit="$(awk -F= '/^--unit=/ { print $2 }' "$at_args")"
: > "$at_stops"
at_clear_out="$(PATH="$at_bin:$PATH" "$GANG" send timed-clear --at clear)"
...
equal "a target with nothing parked says so" "no timed send parked for timed-clear" \
  "$(PATH="$at_bin:$PATH" "$GANG" send timed-clear --at clear)"
...
printf 'MARK_TIMED_ARCHIVED' | PATH="$at_bin:$PATH" \
  "$GANG" send timed-dropped --at 3h --from tester >/dev/null
...
at_noarm_out="$(printf 'MARK_TIMED_NOARM' | GANG_TEST_ARM_FAIL=1 PATH="$at_bin:$PATH" \
  "$GANG" send timed-noarm --at 45m --from tester 2>&1)" || at_noarm_rc=$?
...
printf 'MARK_LINGERS' |
  "$GANG" send lingering --from tester >/dev/null
...
# The state-root resolver is a host-and-uid decision, independent of the
# caller's XDG environment. A logind runtime directory wins when present; the
# fixed per-uid temporary path is the portable fallback.
stateroot_uid="$(id -u)"
stateroot_base="$RUN_ROOT/stateroot"
mkdir -p "$stateroot_base/tmp" "$stateroot_base/run"
stateroot_default="$(GANG_TEST_COLLARS=1 \
  GANG_TEST_STATE_BASES="$stateroot_base/tmp:$stateroot_base/run" sh -c '
    . "$1/libexec/gang-state-root"
    gangline_state_root_resolve "$2"
    printf "%s" "$GANGLINE_STATE_DEFAULT"' sh "$ROOT" "$stateroot_uid")"
equal "without logind the state root is the fixed per-uid fallback" \
  "$stateroot_base/tmp/gangline-$stateroot_uid" "$stateroot_default"
mkdir -m 700 "$stateroot_base/run/$stateroot_uid"
stateroot_default="$(XDG_RUNTIME_DIR="$RUN_ROOT/not-used" GANG_TEST_COLLARS=1 \
  GANG_TEST_STATE_BASES="$stateroot_base/tmp:$stateroot_base/run" sh -c '
    . "$1/libexec/gang-state-root"
    gangline_state_root_resolve "$2"
    printf "%s" "$GANGLINE_STATE_DEFAULT"' sh "$ROOT" "$stateroot_uid")"
equal "logind state is keyed by uid rather than XDG_RUNTIME_DIR" \
  "$stateroot_base/run/$stateroot_uid/gangline" "$stateroot_default"

# A team started while the fixed temporary root was the default remains there
# after logind makes the runtime root preferable. Gangline does not migrate the
# live state, but a command looking in the new root must point at the exact
# configuration line that reaches the old one.
stateroot_session="state-root-moved-$$"
stateroot_other="$stateroot_base/tmp/gangline-$stateroot_uid"
mkdir -p "$stateroot_other/teams"
printf '%s\n' "$stateroot_base/old-team.sock" \
  > "$stateroot_other/teams/$stateroot_session"
stateroot_hint_err="$RUN_ROOT/stateroot-hint.err"
stateroot_hint_out="$(env -u GANG_LOCK_DIR \
  GANG_SESSION="$stateroot_session" \
  GANG_TEST_STATE_BASES="$stateroot_base/tmp:$stateroot_base/run" \
  "$GANG" roster 2> "$stateroot_hint_err")"
contains "a team record under the other possible root is not reported as absent without a route" \
  "$(<"$stateroot_hint_err")" \
  "GANG_LOCK_DIR=$stateroot_other"
contains "the state-root hint names the team whose record was found" \
  "$(<"$stateroot_hint_err")" "$stateroot_session"
contains "and the selected root still reports its immediate state honestly" \
  "$stateroot_hint_out" "no team"
