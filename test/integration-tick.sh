...
tick_prompt() {
  ...
  tmux -S "\$GANG_TMUX_SOCKET" set-option -w -t "\$TMUX_PANE" \
    @gl_turn "closed v2:\$("$ROOT/libexec/gang-clock" now)"
...
tick_cache_ready() {
  ...
  tmux set-option -w -t "$tick_cache_id" @gl_turn "closed v2:$(test_now_ns)"
...
contains "status --why keeps the automatic compaction in a durable journal" \
  "$("$GANG" status --why tick-cache)" "cache compactions:"
contains "status --why renders the automatic compaction's cache age inputs" \
  "$("$GANG" status --why tick-cache)" 'cache stamp '
...
# Cache-compaction configuration is settled when an agent is hitched. A
# malformed environment supplied only to a later tick must therefore neither
# kill the pass nor alter this agent's already-selected TTL and margin.
tick_cache_ready
tick_cache_map_rc=0
...
tick_cache_ready
tmux set-option -w -t "$tick_cache_id" @gl_turn "open v2:$(test_now_ns)"
GANG_CACHE_COMPACTION='tick-cache=120:90' GANG_TEST_TICK_MODE=sync \
  "$GANG" tick >/dev/null
...
# A custom-band candidate reaches the same tick reader as the default one. Put
...
tmux set-option -w -t "$tick_bands_id" @gl_turn "closed v2:$(test_now_ns)"
tmux set-option -w -t "$tick_bands_after_id" @gl_turn "closed v2:$(test_now_ns)"
...
tmux set-option -w -t "$tick_cache_off_id" @gl_turn "closed v2:$(test_now_ns)"
...
tmux set-option -w -t "$tick_cache_bands_low_id" @gl_turn "closed v2:$(test_now_ns)"
...
tmux set-option -w -t "$tick_cache_bands_high_id" @gl_turn "closed v2:$(test_now_ns)"
...
# reason in `gang status --why`; none may silently disable preservation forever.
...
tmux set-option -w -t "$tick_cache_zero_id" @gl_turn "closed v2:$(test_now_ns)"
tmux set-option -w -t "$tick_cache_unreachable_id" @gl_turn "closed v2:$(test_now_ns)"
tmux set-option -w -t "$tick_cache_unreadable_id" @gl_turn "closed v2:$(test_now_ns)"
...
contains "status --why preserves a cache-band decision failure" \
  "$("$GANG" status --why tick-cache-unreadable)" \
...
printf '%s' 'CODEX_MENU_DELIVERY is an ordinary peer message long enough to cross Codex 0.151.0 queue preview limit after wrapping. Its trailing prose must be absent from the three retained rows while the unique Gangline attribution prefix at the head remains visible and proves which single pasted bundle entered the follow-up queue.' \
  | "$GANG" send codex-menu --from tester >/dev/null
...
contains "status --why records the cooperative tick's provider-menu action" \
  "$("$GANG" status --why codex-menu)" \
...
tick_mode_out="$(printf ": > '%s'" "$tick_mode_delivered" |
  ...
  "$GANG" send tick-mode --from tester 2>&1)"
...
printf 'TICK_COPY_MESSAGE' \
  | "$GANG" send tick-copy --from tester >/dev/null
...
printf 'TICK_FALSE_OCCUPIED_MESSAGE' \
  | "$GANG" send tick-false --from tester >/dev/null
...
# already completed.
equal "the post-compaction pass drains the copy-mode-held message" 0 \
  "$("$GANG" roster --porcelain | awk -F '\t' '$1 == "tick-copy" { print $4 }')"
...
equal "a native permission request paints the window occupied" '!tick-glyph!' \
  "$(tmux display-message -p -t "$tick_glyph_id" '#{window_name}')"
contains "and raises the event-backed occupied fact" \
...
equal "every journal line is a five-field record of fixed state words" 0 \
  "$tick_glyph_bad"
contains "status --why shows the agent's recent transitions with their source" \
  "$("$GANG" status --why tick-glyph)" '!occupied! -> ~idle~ (tick)'
...
# enforces, and blank filler lines carry no record for status --why to show. The
...
equal "a rotation that completed marks no journal failure" "" \
  "$(tmux show-options -wqv -t "$tick_glyph_id" @gl_journal_failed)"
contains "status --why reads the crossing edge back from the older generation" \
  "$("$GANG" status --why tick-glyph)" '-busy- -> !occupied! (hook)'
...
# The status --why above repaints the window, and a write of the state a window
...
# A ROSTER SNAPSHOT CAN NAME A WINDOW `gang drop` REMOVES WHILE THE
...
# THE WINDOW MAY DISAPPEAR INSIDE occupied(), after the tick has
...
printf 'TICK_MUST_NOT_REACH_RESTART' \
  | "$GANG" send tick-restart --from tester >/dev/null
tick_identity_rc=0
...
# The same shape seen through the tick: roster and status --why carry no
...
excludes "status --why does not call a sub-threaded session unverified" \
  "$("$GANG" status --why tick-tree-healthy 2>&1)" "identity-unverified"
...
# starting, or one that has not opened its lock -- and those misses must not
# cover the agent's screen or divert keystrokes into a copy-mode overlay.
