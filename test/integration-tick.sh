...
# A COOPERATIVE PASS RETIRES MAIL WHOSE WINDOW DISAPPEARED WITHOUT TEARDOWN.
# The all-window spool register, not the registered-agent roster, is the safety
# boundary: a relaunch may temporarily have its spool identity while its agent
# and collar records are absent, and that live claim must still protect its
# directory. An unreadable register proves nothing and therefore moves nothing.
tick_orphan_token=a263a263a263a263a263a263
tick_orphan_spool="$GANG_LOCK_DIR/spool/$tick_orphan_token"
mkdir -p "$GANG_ARCHIVE_DIR"
mkdir -m 700 "$tick_orphan_spool"
printf 'MARK_TICK_ORPHAN\n' > "$tick_orphan_spool/1-mail"

tick_relaunch_token=b263b263b263b263b263b263
tick_relaunch_spool="$GANG_LOCK_DIR/spool/$tick_relaunch_token"
mkdir -m 700 "$tick_relaunch_spool"
printf 'MARK_TICK_RELAUNCH\n' > "$tick_relaunch_spool/1-mail"
tick_relaunch_id="$(tmux new-window -d -P -F '#{window_id}' \
  -t "=$GANG_SESSION" -n tick-relaunch "PS1='❯ ' exec bash --norc")"
tmux set-option -w -t "$tick_relaunch_id" @gl_spool "$tick_relaunch_token"

"$GANG" tick >/dev/null
equal "tick retires a spool no live window claims" absent \
  "$([ ! -e "$tick_orphan_spool" ] && printf absent || printf present)"
tick_orphan_body="$(find "$GANG_ARCHIVE_DIR" -path "*/orphan-$tick_orphan_token/*" \
  -type f -exec cat {} +)"
contains "tick preserves the orphan's message through the normal archive path" \
  "$tick_orphan_body" MARK_TICK_ORPHAN
equal "tick leaves a relaunching window's claimed spool in place" present \
  "$([ -d "$tick_relaunch_spool" ] && printf present || printf absent)"
contains "and leaves the relaunching window's message untouched" \
  "$(cat "$tick_relaunch_spool/1-mail")" MARK_TICK_RELAUNCH

tick_unreadable_token=c263c263c263c263c263c263
tick_unreadable_spool="$GANG_LOCK_DIR/spool/$tick_unreadable_token"
tick_unreadable_bin="$RUN_ROOT/tick-unreadable-bin"
tick_unreadable_real_tmux="$(command -v tmux)"
mkdir -m 700 "$tick_unreadable_spool"
mkdir -p "$tick_unreadable_bin"
printf 'MARK_TICK_UNREADABLE\n' > "$tick_unreadable_spool/1-mail"
cat > "$tick_unreadable_bin/tmux" <<SH
#!/usr/bin/env bash
. "\$GANG_TEST_PATH_SHIM_GUARD"
...
exec '$tick_unreadable_real_tmux' "\$@"
SH
chmod +x "$tick_unreadable_bin/tmux"
tick_unreadable_rc=0
tick_unreadable_out="$(PATH="$tick_unreadable_bin:$PATH" "$GANG" tick 2>&1)" \
  || tick_unreadable_rc=$?
equal "tick fails when the all-window spool register is unreadable" 1 \
  "$tick_unreadable_rc"
contains "tick names why orphan retirement was refused" \
  "$tick_unreadable_out" "could not read the tmux server's window list"
equal "an unreadable register leaves an unclaimed spool untouched" present \
  "$([ -d "$tick_unreadable_spool" ] && printf present || printf absent)"
contains "and leaves the unreadable spool's message untouched" \
  "$(cat "$tick_unreadable_spool/1-mail")" MARK_TICK_UNREADABLE

tmux kill-window -t "$tick_relaunch_id"
rm -rf -- "$tick_relaunch_spool" "$tick_unreadable_spool"
...
tick_cache_marked_rc=0
tick_cache_marked_out="$(GANG_CACHE_COMPACTION='tick-cache=120:90' GANG_TEST_TICK_MODE=sync \
  "$GANG" tick 2>&1)" || tick_cache_marked_rc=$?
equal "a tick over a window marked safe to drop stays green" 0 "$tick_cache_marked_rc"
equal "a window marked safe to drop is not compacted" 3 "$(tick_cache_count)"
excludes "and no compaction is refused at delivery" "$tick_cache_marked_out" \
  "was not submitted"
tmux set-option -uw -t "$tick_cache_id" @gl_safe_to_drop
