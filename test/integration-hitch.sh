...
# A BARRIER THAT NEEDS THE TMUX SERVER CANNOT REPORT ON THE TMUX SERVER. This
# fixture waits inside that server, so its byte travels down a run-owned pipe
# and neither side is
...
# The shipped Codex directory-trust prompt is an ordinary unanswered prompt: no
...
excludes "hitch no longer asks the operator to send the startup contract" \
  "$startup_gate_out" "gang send startup-gated"
...
# attachment and delivery already proved above. The pipe distinguishes that
# bounded result from a command that never returns.
#
...
# exhaustion; the ledger proves that the budget was consumed rather than spun
# through under a clock that returns instantly.
...
if [ -n "$(window_id hitchbody)" ]; then
  ...
  hitch_body_mail="$("$GANG" queue hitchbody 2>&1)" || hitch_body_mail_rc=$?
...
if [ -n "$(window_id gatedbody)" ]; then
  ...
  gated_body_mail="$("$GANG" queue gatedbody 2>&1)" || gated_body_mail_rc=$?
...
# HITCHED AFTER IT, so it sits later in the window list. An unreadable agent
# must not truncate the listing before this later healthy one.
...
printf '%s\n' 'MARK_CONFIG_LITERAL_LOCK' |
  env -u GANG_LOCK_DIR GANG_CONFIG_DIR="$CONFIG_CASES/literal-lock" \
    "$GANG" send alpha --from config-test >/dev/null
...
mkdir -p "$CONFIG_CASES/bad-env-value"
...
if bad_env_out="$(GANG_BOOT_TIMEOUT=abc GANG_CONFIG_DIR="$CONFIG_CASES/bad-env-value" \
  ...
  fail "a bad environment value names the rejected variable" \
    "hitch unexpectedly succeeded"
else
  contains "a bad environment value names the rejected variable" \
    "$bad_env_out" "GANG_BOOT_TIMEOUT must be a whole number"
fi
...
# The ordinary doctrine contract is taller than the Bash stand-in's original
# 80x24 composer while the explicit overflow world below remains much taller.
# Size only this fixture lane; the prompt fixtures above retain their calibrated
# grid.
...
equal "the cross-session doctrine contract was submitted" "" \
  "$(GANG_SESSION=doctrine-cross-session "$GANG" capture --composer doctrine-cross)"
...
cat > "$RUN_ROOT/collars/startup-prequeued.sh" <<SH
# shellcheck shell=bash
# shellcheck disable=SC2034
. "$ROOT/collars/bash.sh"
...
SH
startup_refusal_rc=0
startup_refusal_out="$("$GANG" hitch startup-refusal \
  -c startup-prequeued -d /tmp 2>&1)" || startup_refusal_rc=$?
equal "a startup delivery refusal preserves the inject refusal status" \
  "3" "$startup_refusal_rc"
contains "a startup delivery refusal names the bounded rollback" \
  "$startup_refusal_out" \
  "gang: startup contract to 'startup-refusal' was not delivered after"
...
red_time="$(printf '%s' '{"hook_event_name":"PostToolUse"}' |
  GANG_TEST_CLOCK="$curfew_clock_shim" GANG_TEST_CLOCK_NOW_NS=125000000000 \
  TMUX_PANE="$(tmux list-panes -t "$(window_id alpha)" -F '#{pane_id}')" \
  "$GANG" hook)"
...
printf 'MARK_ALPHA' | "$GANG" send alpha --from tester >/dev/null
...
printf 'MARK_INSIDE_SENDER' | TMUX_PANE="$alpha_tmux_pane" \
  "$GANG" send inside-target >/dev/null
...
# environment stripped, so `self_window` finds nothing and `--from` becomes
# mandatory. Nothing here proves who is
...
declared_out="$(printf 'MARK_DECLARED_SENDER' | env -u TMUX -u TMUX_PANE \
  "$GANG" send declared-target --from alpha)"
...
printf '%s\n' '{"hook_event_name":"PermissionRequest"}' \
  | GANG_NOTIFY=amb TMUX_PANE="$alpha_tmux_pane" "$GANG" hook \
...
if printf 'MARK_GHOST' | "$GANG" send ghost --from tester >/dev/null 2>&1; then
...
# @gl_session_id, through collar_session_id. A collar with that witness but no
# resume launch must not print an unusable relaunch command. Stamped directly here
...
printf 'MARK_SCOPED' | "$GANG" send scoped --from tester >/dev/null
...
# READS REPORT EXACT-SHAPED ORPHANS AND MUTATE NONE. The first unit came from
# a hitch whose window then died. The second proves that shape, rather than a
# second registry, is the ownership boundary.
PATH="$RUN_ROOT/scope-orphan-bin:$PATH" GANG_SCOPE=on \
...
scope_lookalike_unit="gangline-$GANG_SESSION-lookalike-0123456789abcdef.scope"
scope_description_unit="gangline-$GANG_SESSION-description-0123456789abcdef.scope"
: > "$RUN_ROOT/scope-orphan-active/$scope_lookalike_unit"
...
scope_roster_out="$(PATH="$RUN_ROOT/scope-orphan-bin:$PATH" \
  "$GANG" roster)"
contains "status reports a nonce-bearing scope with no live window" \
...
contains "status reports a second exact-shaped orphan without a registry lookup" \
  "$scope_status_out" "orphaned scope $scope_lookalike_unit"
contains "roster reports the second exact-shaped orphan too" \
  "$scope_roster_out" "orphaned scope $scope_lookalike_unit"
...
equal "the second exact-shaped orphan remains active after read-only commands" "$scope_lookalike_unit" \
  "$(find "$RUN_ROOT/scope-orphan-active" -maxdepth 1 -type f -name "$scope_lookalike_unit" -printf '%f')"
...
scope_rollback_out="$(PATH="$RUN_ROOT/scope-orphan-bin:$PATH" GANG_SCOPE=on \
  "$GANG" hitch scope-rollback -c startup-prequeued -d /tmp 2>&1)" \
  || scope_rollback_rc=$?
...
equal "status --why maps the renamed registration to its exact scope" \
  "scope: $scope_kept_unit" \
  "$("$GANG" status --why scopekept | sed -n '/^scope:/p')"
...
# Every agent gets a unit, but only the `tmux new-session` that forks a server
# may be wrapped —
...
# A LAUNCH THAT DIES IS NOT A SLOW BOOT. Whether it dies before registration or
# while Gangline waits for its input box, the result must name the failed launch
# rather than offer recovery for a live agent.
