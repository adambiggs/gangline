...
# SYNTHETIC HISTORY REPLAYS THROUGH THE SAME ENGINE THAT WATCHES LIVE.
cap_pass "synthetic history replays without refusing" 0 \
  "$ROOT/libexec/gang-cap" __replay \
...
equal "four synthetic windows produce two alerts and no others" \
  "alerts: 2" "$(printf '%s\n' "$cap_replayed" | tail -1)"
...
refuses "a replay row that records no reading cannot supply a percentage" \
  "line 1" "$ROOT/libexec/gang-cap" __replay --readings "$cap_forged_readings" --thresholds 70
