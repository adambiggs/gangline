...
GANG_STOP_HOOK=1
GANG_BUSY_REGEX='STATUS_WHY_BUSY_[0-9]+'
...
tmux set-option -w -t "$waitable_id" @gl_turn "open v2:$(test_now_ns)"
...
# STATUS --WHY INSTRUMENTS THIS STATE READ, rather than taking a diagnostic capture
...
status_why_painted="gang-test-status-why-painted-$$"
printf -v status_why_command \
  "printf 'STATUS_WHY_BUSY_%%s\\\\n' \"\$((6*7))\"; tmux wait-for -S %q" \
  "$status_why_painted"
tmux send-keys -l -t "$waitable_pane" "$status_why_command"
tmux send-keys -t "$waitable_pane" Enter
tmux wait-for "$status_why_painted"
status_why_out="$("$GANG" status --why waitable)"
contains "status --why reports the live state its rules produced" \
  "$status_why_out" "state: -busy-"
contains "status --why names the collar busy rule that matched" \
  "$status_why_out" "GANG_BUSY_REGEX: matched"
contains "status --why prints the exact busy regex" \
  "$status_why_out" "rule: STATUS_WHY_BUSY_[0-9]+"
contains "status --why prints the pane fragment that matched" \
  "$status_why_out" "fragment: STATUS_WHY_BUSY_42"
contains "status --why distinguishes a tested occupancy miss" \
  "$status_why_out" "GANG_OCCUPIED_REGEX: did not match"
contains "status --why distinguishes a tested fatal-evidence miss" \
  "$status_why_out" "collar_bricked: did not match"
...
occupied_explain="$("$GANG" status --why waitable)"
contains "status --why records the occupancy rule that outranked fatal evidence" \
  "$occupied_explain" "GANG_OCCUPIED_REGEX: matched"
contains "status --why leaves fatal evidence unevaluated after occupancy settles state" \
...
brick_explain="$("$GANG" status --why waitable)"
contains "status --why names the collar fatal reader that matched" \
  "$brick_explain" "collar_bricked: matched"
contains "status --why prints the collar's fatal-turn cause" \
...
# every guard gang keeps on the input box: without this check the pane reads
...
contains "a repainted busy marker is the live state before the blocked check" \
...
equal "porcelain gives the blocked state its stable word" "blocked" \
  "$("$GANG" roster --porcelain | awk -F '\t' '$1 == "waitable" { print $3 }')"
blocked_explain="$("$GANG" status --why waitable)"
contains "status --why names the collar blocked reader that matched" \
  "$blocked_explain" "collar_blocked: matched"
contains "status --why prints the collar's blocking reason" \
...
blocked_send_out="$(printf 'BLOCKED_BODY' | "$GANG" send \
  waitable --from tester --live-only 2>&1)" || blocked_send_rc=$?
...
# own check, before the one inject keeps, so without the same ordering there it
...
contains "status --why leaves the blocked reader unevaluated once fatal evidence settles state" \
  "$("$GANG" status --why waitable)" "collar_blocked: not evaluated"
rm -f -- "$RUN_ROOT/brick-evidence"
...
tmux set-option -w -t "$wait_natural_id" @gl_turn "open v2:$(test_now_ns)"
...
SH
chmod +x "$wait_clock"
tmux set-option -w -t "$waitable_id" @gl_turn "open v2:1000000000"
wait_deadline_arm="gang-test-wait-arm-$$-deadline"
wait_deadline_pids="$RUN_ROOT/wait-deadline-pids"
wait_deadline_done="$RUN_ROOT/wait-deadline-done"
(
  wait_deadline_child_rc=0
  BASH_ENV="$RUN_ROOT/wait-arm-env" GANG_TEST_WAIT_ARM="$wait_deadline_arm" \
    ...
    || wait_deadline_child_rc=$?
  printf '%s\n' "$wait_deadline_child_rc" > "$wait_deadline_done"
  tmux wait-for -S "$wait_deadline_arm"
  exit "$wait_deadline_child_rc"
) &
wait_deadline_pid=$!
tmux wait-for "$wait_deadline_arm"
if [ -e "$wait_deadline_done" ]; then
  wait_deadline_rc="$(<"$wait_deadline_done")"
  wait "$wait_deadline_pid" || :
  fail "the deadline fixture reaches the native wait edge" \
    "gang wait returned $wait_deadline_rc before arming: $(<"$RUN_ROOT/wait-deadline.err")"
else
...
# Stall lights forward only native awaiting-input witnesses to the configured
# target. Every outcome is synchronous: the hook returns after the
# note is accepted live, parked, or recorded as failed.
stall_original_notify="${GANG_NOTIFY-}"
stall_had_notify="${GANG_NOTIFY+x}"
...
export GANG_NOTIFY=stall-target
...
export GANG_NOTIFY=stall-raise
...
tmux new-window -d -t "=$GANG_SESSION" -n stall-unadopted -c /tmp \
  "PS1='❯ ' bash --norc"
export GANG_NOTIFY=stall-unadopted
...
export GANG_NOTIFY=stall-gated
...
if [ -n "$stall_had_notify" ]; then
  export GANG_NOTIFY="$stall_original_notify"
else
  unset GANG_NOTIFY
fi
...
# Provider-limit commands use a collar's non-interactive correctness source.
# The native rows carry their own observation and reset clocks.
usage_limits_source="$RUN_ROOT/usage-limits.rows"
cat > "$RUN_ROOT/collars/usage-limits.sh" <<SH
# shellcheck shell=bash
# shellcheck disable=SC2034
. "$ROOT/collars/bash.sh"
GANG_STOP_HOOK=1
GANG_USAGE_LIMIT_INTERVAL=60
...
SH
usage_now="$(date +%s)"
"$HITCH" usage-reader -c usage-limits -d /tmp >/dev/null
usage_reader_id="$(window_id usage-reader)"
usage_reader_pane="$(tmux list-panes -t "$usage_reader_id" -F '#{pane_id}')"
printf '%s\n' \
  "Current session"$'\t'"95"$'\t'"$(( usage_now + 600 ))"$'\t'"$usage_now" \
  "Current week"$'\t'"85"$'\t'"$(( usage_now + 86400 ))"$'\t'"$usage_now" \
  > "$usage_limits_source"
usage_limits_out="$("$GANG" limits usage-reader)"
...
printf '%s\n' \
  ...
  > "$usage_limits_source"
sanitized_usage_limits="$("$GANG" limits usage-reader)"
...
"$HITCH" usage-absent -c stallable -d /tmp >/dev/null
...
SH
"$HITCH" usage-bound-fail -c usage-bound-fail -d /tmp >/dev/null
...
# that window.
tmux show-options -wqv -t "$usage_reader_id" @gl_usage_wake \
...
usage_wait_out="$(PATH="$usage_timer_bin:$PATH" "$GANG" wait --limit usage-reader \
  --resume 'Resume the assigned provider-limit fixture.')"
...
usage_wake_record="$(tmux show-options -wqv -t "$usage_reader_id" @gl_usage_wake)"
...
equal "the timer callback is gang's own wait --limit" "yes" \
  "$(case "$(tail -n 8 "$usage_timer_args" | head -n 1)" in
       */bin/gang) printf yes ;; *) printf no ;; esac)"
equal "and its tail carries the reset and the unit it was armed as" \
  "wait --limit usage-reader --fire $usage_wake_reset --unit $usage_wake_unit" \
  "$(tail -n 7 "$usage_timer_args" | tr '\n' ' ' | sed 's/ $//')"
...
printf '%s' '{"hook_event_name":"Stop"}' |
  TMUX_PANE="$usage_reader_pane" "$GANG" hook >/dev/null
PATH="$usage_timer_bin:$PATH" "$GANG" wait --limit usage-reader \
...
refuses "an internal callback with no unit is refused" \
  ...
  env PATH="$usage_timer_bin:$PATH" "$GANG" wait --limit usage-reader \
    --fire "$usage_wake_reset"
GANG_TEST_NOW="$usage_wake_reset" PATH="$usage_timer_bin:$PATH" \
  "$GANG" wait --limit usage-reader --fire "$usage_wake_reset" \
...
excludes "and delivers nothing into the window it was aimed at" \
  "$(pane_all usage-reader)" "Resume the assigned provider-limit fixture."
...
: > "$usage_timer_stops"
PATH="$usage_timer_bin:$PATH" "$GANG" wait --limit usage-reader >/dev/null
usage_clear_unit="$(tmux show-options -wqv -t "$usage_reader_id" @gl_usage_wake |
  cut -f2)"
: > "$usage_timer_stops"
PATH="$usage_timer_bin:$PATH" "$GANG" wait --limit usage-reader --clear >/dev/null
...
: > "$usage_timer_stops"
PATH="$usage_timer_bin:$PATH" "$GANG" wait --limit usage-reader >/dev/null
usage_arm_unit="$(tmux show-options -wqv -t "$usage_reader_id" @gl_usage_wake |
  cut -f2)"
...
contains "a reset during arming is delivered, not stranded" \
  "$(pane_all usage-reader)" "RACE_RESUME_BODY reached the agent."
...
GANG_TEST_TMUX_FAIL=@gl_usage_wake PATH="$usage_timer_bin:$PATH" \
  "$GANG" wait --limit usage-reader --resume 'never stored' >/dev/null 2>&1 \
  || usage_store_rc=$?
...
equal "and leaves no orphan resume body" "" \
  "$(tmux show-options -wqv -t "$usage_reader_id" @gl_usage_wake_body)"
usage_body_rc=0
GANG_TEST_TMUX_FAIL=@gl_usage_wake_body PATH="$usage_timer_bin:$PATH" \
  "$GANG" wait --limit usage-reader --resume 'never stored either' >/dev/null 2>&1 \
  || usage_body_rc=$?
...
GANG_TEST_SYSTEMCTL=gone PATH="$usage_timer_bin:$PATH" \
  "$GANG" wait --limit usage-reader --clear >/dev/null 2>&1 || usage_gone_rc=$?
...
usage_stuck_err="$(GANG_TEST_SYSTEMCTL=stuck PATH="$usage_timer_bin:$PATH" \
  "$GANG" wait --limit usage-reader --clear 2>&1 >/dev/null)" || usage_stuck_rc=$?
...
usage_blind_err="$(GANG_TEST_SYSTEMCTL=blind PATH="$usage_timer_bin:$PATH" \
  "$GANG" wait --limit usage-reader --clear 2>&1 >/dev/null)" || usage_blind_rc=$?
...
#
# THE THRESHOLD IS A HITCH-TIME CHOICE, and this agent exercises the native
# sampling path that turns it into one reset wake.
auto_now="$(date +%s)"
auto_reset=$(( auto_now + 900 ))
printf '%s\t%s\t%s\t%s\n' \
  "Current session" 96 "$auto_reset" "$auto_now" \
  > "$usage_limits_source"
GANG_AUTO_RESUME=95% "$HITCH" auto-res -c usage-limits -d /tmp >/dev/null
...
auto_throttled="$(printf '%s' '{"hook_event_name":"PostToolUse"}' |
  PATH="$usage_timer_bin:$PATH" TMUX_PANE="$auto_pane" "$GANG" hook)"
equal "a nearby auto-resume hook emits no repeated provider result" "" \
  "$auto_throttled"
...
contains "the provider-reader throttle uses the shared monotonic clock" \
  "$(tmux show-options -wqv -t "$auto_id" @gl_usage_checked)" "v2:"
...
tmux set-option -w -t "$auto_id" @gl_usage_checked "v2:1"
auto_calls_before_unknown="$(tmux show-options -wqv -t "$auto_id" @test_usage_calls)"
auto_unknown_clock="$(printf '%s' '{"hook_event_name":"PostToolUse"}' |
  GANG_TEST_CLOCK="$usage_clock_unknown" PATH="$usage_timer_bin:$PATH" \
    TMUX_PANE="$auto_pane" "$GANG" hook)"
equal "an unknown throttle clock emits no provider result" "" \
  "$auto_unknown_clock"
equal "an unknown throttle clock does not sample the provider" \
  "$auto_calls_before_unknown" \
  "$(tmux show-options -wqv -t "$auto_id" @test_usage_calls)"
...
GANG_AUTO_RESUME=95% "$HITCH" auto-claim -c usage-limits -d /tmp >/dev/null
...
# THE POINT OF THE WHOLE SCENARIO, and it consumes THE DECLARATION THE HOOK JUST
...
printf '%s\t%s\t%s\t%s\n' \
  ...
  > "$usage_limits_source"
"$HITCH" auto-off -c usage-limits -d /tmp >/dev/null
...
# A SOURCE THAT CANNOT ANSWER IS NAMED FOR AUTO-RESUME.
...
contains "an unavailable source names auto-resume" \
...
# A GOOD AUTO-RESUME SAMPLE ENDS A READER-FAILURE EPOCH. A later recurrence is
# therefore loud again.
tmux set-option -uw -t "$auto_id" @gl_auto_resume_limit_notified 2>/dev/null || true
: > "$usage_limits_source"
tmux set-option -w -t "$auto_id" @gl_usage_checked "$(( auto_now - 60 ))"
...
if auto_high="$(GANG_AUTO_RESUME=101% "$GANG" hitch \
    auto-invalid -c usage-limits -d /tmp 2>&1)"; then
...
exit 17
SH
if PATH="$usage_timer_bin:$PATH" "$GANG" wait --limit usage-reader >/dev/null 2>&1; then
...
"$GANG" drop usage-reader >/dev/null
...
printf '%s\n' 0.7.0 > "$installer_src/version.txt"
...
git -C "$installer_src" tag gangline-v0.7.0
printf '%s\n' 0.8.0 > "$installer_src/version.txt"
git -C "$installer_src" commit -qam 'test: newer installer release'
git -C "$installer_src" tag gangline-v0.8.0
printf '%s\n' 0.9.0 > "$installer_src/version.txt"
...
PATH="$installer_tmux_bin:$PATH" GANG_TEST_TMUX_VERSION=3.2 \
  ...
  GANGLINE_HOME="$installer_root/installed" GANGLINE_BIN="$installer_bin" \
...
equal "the installer replaces a symlinked destination instead of writing through it" \
  "0 absent $installer_root/installed/bin/gang" \
...
equal "the installer selects the latest stable release rather than repository HEAD" \
  "0.8.0 tagged" \
  "$(cat "$installer_root/installed/version.txt") $([ "$(git -C "$installer_root/installed" rev-parse HEAD)" = "$(git -C "$installer_src" rev-list -n 1 gangline-v0.8.0)" ] && printf tagged || printf other)"
...
installed_home="$installer_root/installed"
...
equal "gang upgrade --check reports both sides of a current release" \
  "gang is current: v0.8.0 -> v0.8.0 (already at latest release)" \
...
git -C "$installer_src" tag gangline-v0.9.0
...
equal "gang upgrade --check reports a minor update without inventing a release count" \
  "upgrade available: v0.8.0 -> v0.9.0 (minor update)" \
...
contains "gang upgrade reports its installed and selected releases before replacing files" \
  "$available_upgrade_out" "upgrading from v0.8.0 -> v0.9.0"
equal "gang upgrade installs the available release over the current install" \
  "0.9.0 tagged" \
  "$(cat "$installer_root/installed/version.txt") $([ "$(git -C "$installer_root/installed" rev-parse HEAD)" = "$(git -C "$installer_src" rev-list -n 1 gangline-v0.9.0)" ] && printf tagged || printf other)"

unknown_home="$installer_root/unknown-home"
git clone -q "$installer_src" "$unknown_home"
git -C "$unknown_home" checkout --detach -q gangline-v0.9.0
...
git -C "$malformed_home" checkout --detach -q gangline-v0.9.0
...
git -C "$multiline_version_home" checkout --detach -q gangline-v0.9.0
...
git -C "$whitespace_version_home" checkout --detach -q gangline-v0.9.0
...
git -C "$newer_home" checkout --detach -q gangline-v0.9.0
...
equal "gang upgrade --check names a current-newer installation without downgrading" \
  "installed version is newer than latest: v9.0.0 -> v0.9.0; no changes made" \
  "$newer_release_check"
newer_upgrade_rc=0
...
equal "gang upgrade refuses to replace a current-newer installation" \
  ...
  "$([ "$newer_upgrade_rc" -ne 0 ] && printf refused || printf downgraded) $([[ "$newer_upgrade_out" = *'refusing downgrade from installed v9.0.0 -> selected v0.9.0'* ]] && printf named || printf unnamed) $(cat "$newer_home/version.txt") $(git -C "$newer_home" rev-parse HEAD)"
...
unstable_rc=0
unstable_out="$(GANGLINE_REPO="$unstable_src" GANGLINE_HOME="$installer_root/installed" \
...
unreachable_out="$(GANGLINE_REPO="$installer_root/unreachable-source" \
  GANGLINE_HOME="$installer_root/installed" sh "$ROOT/install.sh" --check 2>&1)" \
  || unreachable_rc=$?
...
# BREAKING CHANGE: callers must pass the recipient positionally
')"
...
# that OPENS the composer and leaves the closing rule pure. The reader must
# distinguish that selected-child box from an absent composer. On 2.1.241,
# typing into that box while an in-process
...
refuses "gang capture --composer names the box as a subagent's rather than reporting none" \
  "belongs to a selected in-process subagent" \
  "$GANG" capture --composer subframe

printf 'MARK_SUBAGENT_FRAME' |
  "$GANG" send subframe --from tester >/dev/null
...
parentframe_composer="$("$GANG" capture --composer parentframe 2>&1)" \
...
refuses "gang capture --composer names the background-sessions box" \
  "would create a new session instead of reaching this agent" \
  "$GANG" capture --composer backgroundframe
background_send_rc=0
background_send="$(printf 'MARK_BACKGROUND_VIEW' |
  "$GANG" send backgroundframe --from tester --live-only 2>&1)" \
  || background_send_rc=$?
...
refuses "a cursor resting on main does not make the child composer the agent's" \
  "belongs to a selected in-process subagent" \
  "$GANG" capture --composer cursorframe
...
printf 'MARK_EXPIRY_FREE' |
  "$GANG" send freeagent --from tester >/dev/null
...
tmux set-option -w -t "$freeagent_id" @gl_turn "open v2:1"
...
printf 'MARK_EXPIRY_HELD' |
  "$GANG" send heldagent --from tester >/dev/null
tmux set-option -w -t "$heldagent_id" @gl_turn "open v2:1"
...
printf 'MARK_TEARDOWN_ARCHIVE' |
  env GANG_SESSION="$teardown_session" "$GANG" send \
    spoolable --from tester >/dev/null
