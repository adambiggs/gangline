...
printf 'MARK_NUMERIC' | "$GANG" send 1 --from tester >/dev/null
...
printf -v self_send_command \
  'printf MARK_SELF_ATTRIBUTED | %q send 1 >/dev/null; tmux wait-for -S %q' \
  "$GANG" "$self_sent"
...
printf -v claimed_send_command \
  'printf MARK_FALSE_CLAIM | %q send 1 --from impostor >/dev/null 2>&1; tmux set-option -w @test_sender_rc "$?"; tmux wait-for -S %q' \
  "$GANG" "$claimed_sent"
...
if printf 'MARK_UNSIGNED' | "$GANG" send 1 >/dev/null 2>&1; then
...
alpha_lock_identity="$("$ROOT/libexec/gang-process-identity" --tick "$$" "$GANG_SESSION")"
IFS=$'\t' read -r _ alpha_lock_token _ _ _ _ _ alpha_lock_namespace \
  <<<"$alpha_lock_identity"
alpha_live_lock="v1:$$:$alpha_lock_token:$alpha_lock_namespace"
alpha_dead_lock="v1:99999999:0:$alpha_lock_namespace"
mkdir -p "$GANG_LOCK_DIR"
ln -s "$alpha_live_lock" "$alpha_delivery_lock"
...
if live_lock_refusal="$(printf 'MARK_LIVE_LOCK' |
  GANG_LOCK_WAIT=not-a-number GANG_TEST_CLOCK_LEDGER="$live_lock_ledger" \
    "$GANG" send alpha --from tester 2>&1)"; then
  live_lock_rc=0
else
  live_lock_rc=$?
fi
...
ln -s "$alpha_dead_lock" "$alpha_delivery_lock"
printf 'MARK_STALE_LOCK' |
  "$GANG" send alpha --from tester >/dev/null
...
if printf 'MARK_RELEASE_BETWEEN_L_AND_READLINK' |
  PATH="$lock_release_race_bin:$PATH" "$GANG" send alpha --from tester >/dev/null; then
...
if bounded_lock_out="$(printf 'MARK_BOUND_LOCK_RACE' |
  PATH="$lock_bound_race_bin:$PATH" "$GANG" send alpha --from tester 2>&1)"; then
...
if "$GANG" capture --composer 1 >/dev/null; then
  pass "the draft inspection command runs"
else
  fail "the draft inspection command runs" "gang capture --composer 1 failed"
fi
contains "capture --composer prints what a human typed" \
  "$("$GANG" capture --composer 1)" "HUMAN_DRAFT"
...
strand_out="$(printf ": > '%s'" "$queue_executed" |
  "$GANG" send strand --from tester 2>&1)" || strand_rc=$?
...
if strand_more="$(printf 'MARK_SECOND' | "$GANG" send strand --from tester 2>&1)"; then
  strand_more_rc=0
else
  strand_more_rc=$?
fi
...
if strand_live="$(printf 'MARK_LIVE_QUEUE' |
  "$GANG" send strand --from tester --live-only 2>&1)"; then
  strand_live_rc=0
else
  strand_live_rc=$?
fi
...
panepark_out="$(printf 'MARK_PANE_PARK' |
  "$GANG" send panepark --from tester 2>&1)" || panepark_rc=$?
...
panepark_unknown="$(printf 'MARK_PANE_UNKNOWN' |
  "$GANG" send panepark --from tester --live-only 2>&1)" \
  || panepark_unknown_rc=$?
...
# both leaks the suite's envelopes into it and lets ambient history reach the
# recall fixture.
...
if redraw_out="$(printf '%s\n' \
  ...
  "$GANG" send parked --from tester 2>&1)"; then
...
if printf 'MARK_MULTI head\nMARK_MULTI_TAIL' |
  "$GANG" send parked --from tester >/dev/null 2>&1; then
...
: > "$RUN_ROOT/flush-arm"
if printf 'MARK_PARKED' | "$GANG" send parked --from tester >/dev/null 2>&1; then
...
tmux set-option -w -t "$parked_id" @gl_turn "open v2:$(test_now_ns)"
...
tmux set-option -w -t "$parked_id" @gl_turn "closed v2:$(test_now_ns)"
: > "$RUN_ROOT/flush-drain"
...
bracket_hook Stop                       # a closed turn before compaction begins
...
bracket_out="$(printf 'MARK_MIDCOMPACT' \
  | "$GANG" send bracket --from tester 2>&1)" || :
...
# means refusing every multiline recovery, which is the defect. The cost: whitespace-only substitutions are indistinguishable, and no
...
# is read back against. A composer reading can normalize whitespace and cannot
# stand in for the submitted bytes.
: > "$RUN_ROOT/flush-drain"
flush_settle
: > "$RUN_ROOT/flush-arm"
...
if printf 'MARK_FROZEN_RECALL' |
  "$GANG" send frozenrecall --from tester >/dev/null 2>&1; then
...
tmux set-option -w -t "$steer_id" @gl_turn "open v2:$(test_now_ns)"
: > "$RUN_ROOT/steer-claim-watch"
if steer_out="$(printf 'MARK_STEER' | "$GANG" send steer --from tester 2>&1)"; then
  steer_rc=0
else
  steer_rc=$?
fi
...
if printf 'MARK_STEER_PARK' |
  "$GANG" send steer --from tester >/dev/null 2>&1; then
  steer_park_rc=0
else
  steer_park_rc=$?
fi
...
tmux set-option -w -t "$steer_id" @gl_turn "closed v2:$(test_now_ns)"
...
compact_steer_out="$(printf 'MARK_COMPACT_STEER' |
  "$GANG" send steer --from tester 2>&1)"
...
tmux set-option -w -t "$steer_id" @gl_turn "open v2:$(test_now_ns)"
if steer_live="$(printf 'MARK_LIVE' | "$GANG" send steer --from tester --live-only 2>&1)"; then
  steer_live_rc=0
else
  steer_live_rc=$?
fi
...
# PostToolUse drains while @gl_turn stays open.
...
tmux set-option -w -t "$park_only_id" @gl_turn "open v2:$(test_now_ns)"
park_only_out="$(printf 'MARK_PARK_ONLY' |
  "$GANG" send park-only --from tester 2>&1)"
...
frozenbox_out="$(printf 'MARK_FROZEN_BOX' |
  "$GANG" send frozenbox --from tester 2>&1)" || frozenbox_rc=$?
...
tmux set-option -w -t "$stop_id" @gl_turn "open v2:$(test_now_ns)"
...
tmux set-option -w -t "$stubborn_id" @gl_turn "open v2:$(test_now_ns)"
...
if midturn_out="$(printf 'MARK_MIDTURN' |
  "$GANG" send stubborn --from tester --live-only 2>&1)"; then
