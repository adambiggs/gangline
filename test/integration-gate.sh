...
cp -R "$ROOT/libexec" "$gate_fix/libexec"
printf 'ignored.txt\n' > "$gate_fix/.gitignore"
printf 'DOOMED\n' > "$gate_fix/doomed.txt"
...
cp -R "$ROOT/libexec" "$gate_twin/libexec"
printf 'ignored.txt\n' > "$gate_twin/.gitignore"
...
if [ "${GANG_INTEGRATION_REQUIRE_ALL_PROBE:-0}" != 1 ]; then
  ...
  contains "the required run names every part cli omitted" \
    ...
    "required full run omitted declared parts: substrate,trust,hitch,compose,spool,readiness,hooks,notify,usage,events,cap,tick,friction,run"
