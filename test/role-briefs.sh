...
submitted() { equal "$1" "" "$("$GANG" capture --composer "$2")"; }
...
ac1() {
  ...
  equal "AC1 the startup contract was submitted" "" \
    "$("$GANG" capture --composer role-ac1)"
  drop_agent role-ac1
}
...
ac11() {
  ...
  value="$(<"$prefix.2.bin")"
  if printf '%s' "$value" | python3 -c 'import sys; s=sys.stdin.read(); raise SystemExit(0 if s.index("--- Gangline contract") < s.index("OPERATOR_BODY_WITHOUT_PREAMBLE") else 1)'; then
...
ac12() {
  local config="$TEST_ROOT/ac12-config" prefix="$TEST_ROOT/ac12-argv" value
  mkdir -p "$config/roles"
  ...
  value="$(<"$prefix.2.bin")"
  contains "AC12 quoted role reaches one argv value" "$value" "quote ' here"
  contains "AC12 dollar reaches the same argv value" "$value" '$dollar'
...
ac22() {
  ...
  equal "AC22 the default role contract was submitted" "" \
    "$("$GANG" capture --composer role-ac22-default)"
  ...
  equal "AC22 the override contract was submitted" "" \
    "$("$GANG" capture --composer role-ac22-override)"
...
ac26() {
  ...
  if [ "$rc" -eq 0 ]; then
    ...
    equal "AC26 the worker startup contract was submitted" "" \
      "$("$GANG" capture --composer role-ac26)"
