...
run_small_mail="$(XDG_STATE_HOME="$run_state" "$GANG" queue run-requester)"
...
run_locked_identity="$("$ROOT/libexec/gang-process-identity" --tick "$$" "$GANG_SESSION")"
IFS=$'\t' read -r _ run_locked_token _ _ _ _ _ run_locked_namespace \
  <<<"$run_locked_identity"
ln -s "v1:$$:$run_locked_token:$run_locked_namespace" "$run_requester_lock"
...
run_rename_mail="$(XDG_STATE_HOME="$run_state" "$GANG" queue run-requester-renamed)"
