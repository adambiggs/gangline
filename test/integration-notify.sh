...
notify_original_target="${GANG_NOTIFY-}"
notify_had_target="${GANG_NOTIFY+x}"
...
contains "status --why distinguishes a collar with no root probe" \
  "$("$GANG" status --why state-lead)" "harness identity: collar declares no probe"
...
export GANG_NOTIFY=state-returned
printf '%s' yes > "$RUN_ROOT/state-notify-blocked"
printf '%s' '{"hook_event_name":"Notification","notification_type":"idle_prompt"}' \
...
contains "status --why names an unestablished declared root probe" \
  "$("$GANG" status --why state-codex-delayed)" "harness identity: not yet established"
...
PY
  ...
  contains "status --why makes the recorded liveness coverage visible" \
    "$("$GANG" status --why state-codex)" "harness identity: recorded"
  ...
  contains "a replaced recorded harness is named harness-lost, not dead" \
    "$("$GANG" status state-codex)" "!harness-lost!"
  contains "status --why distinguishes a lost identity from unavailable coverage" \
    "$("$GANG" status --why state-codex)" "harness identity: lost"
  ...
  contains "status --why makes unestablished liveness coverage visible" \
    "$("$GANG" status --why state-codex)" "harness identity: not yet established"
fi
...
if [ -n "$notify_had_target" ]; then
  export GANG_NOTIFY="$notify_original_target"
else
  unset GANG_NOTIFY
fi
