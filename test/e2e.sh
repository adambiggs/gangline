...
STUB_API_KEY=e2e-placeholder-key
...
say() { # $1 prompt text -> submitted as the operator
  printf '%s' "$1" | "$GANG" send "$AGENT" --from operator
}
...
scenario_peer() {
  ...
  printf '%s' "$request_body" \
    | TMUX_PANE="$peer_pane" "$GANG" send "$AGENT" \
...
scenario_bricked() {
  ...
  if [ "$reading" != bricked ]; then
    ...
    note "$("$GANG" status --why "$AGENT" 2>&1 | tail -6)"
  fi
...
scenario_midturn() {
  ...
  printf 'read this: %s' "$token" \
    | "$GANG" send "$AGENT" --from courier >"$RUN_ROOT/send.out" 2>&1 \
    || send_rc=$?
...
scenario_blocked() {
  ...
  if [ "$reading" != blocked ]; then
    note "RECORDED on $HARNESS_BUILD: it read '$reading' instead."
    note "$("$GANG" status --why "$AGENT" 2>&1 | tail -8)"
  fi
  ...
  unread_explain="$(GANG_COLLARS="$RUN_ROOT/collars-unread" "$GANG" status --why "$AGENT" 2>&1)"
  ...
  send_out="$(printf 'E2E-BLOCKED-BODY' | "$GANG" send "$AGENT" --from operator \
    --live-only 2>&1)" || send_rc=$?
  ...
  excludes "blocked: nor is the occupancy check answering" \
  ...
  excludes "blocked: the fatal reader does not also claim this turn" \
    "$("$GANG" status --why "$AGENT" 2>&1)" "collar_bricked: matched"
  stub_sound blocked
  world_down
}
