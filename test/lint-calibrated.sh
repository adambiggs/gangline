...
#
# The lint rules that prove themselves against planted cases before judging
# the tree. test/lint.sh sources this in its full run, which CI makes; the
# local gate skips it.

# THE INTERPRETER RULE IN test/suite-python.sh IS CALIBRATED BEFORE IT IS
# TRUSTED. On a host whose python3 is already an ordinary binary the rule is a
# no-op, so a green run there says
...
ceiling_awk='
  FILENAME ~ /lint-calibrated\.sh$/ { next }     # the checker has to name what it refuses
  /^[[:space:]]*#/ { next }
  { line = $0
    hash = index(line, " #")
