...
tmux set-option -w -t "$(window_id 1)" @gl_staged_box "$("$GANG" capture --composer 1)"
...
if printf 'MARK_RETAIN' | "$GANG" send 1 --from tester --live-only >/dev/null 2>&1; then
...
tmux send-keys -t "$(window_id 1)" C-u
printf 'MARK_CLEARS' | "$GANG" send 1 --from tester >/dev/null
...
if drain_out="$(printf 'MARK_DRAIN' | "$GANG" send drain --from tester 2>&1)"; then
...
if flicker_out="$(printf 'MARK_FLICKER' | "$GANG" send flicker --from tester 2>&1)"; then
...
readiness_turn_clock="$RUN_ROOT/readiness-turn-clock"
cat > "$readiness_turn_clock" <<'SH'
#!/bin/sh
case "$1" in
  now) printf '%s\n' 400000000001 ;;
...
SH
chmod +x "$readiness_turn_clock"
readiness_saved_clock_is_set=${GANG_TEST_CLOCK+x}
readiness_saved_clock=${GANG_TEST_CLOCK-}
export GANG_TEST_CLOCK="$readiness_turn_clock"
stale_bracket="open v2:1"
tmux set-option -w -t "$(window_id 1)" @gl_turn "$stale_bracket"
if printf 'MARK_TURNFALL' | "$GANG" send 1 --from tester >/dev/null 2>&1; then
...
tmux set-option -w -t "$(window_id 1)" @gl_turn "open v2:1"
tmux send-keys -l -t "$(window_id 1)" 'half a draft'
if veto_draft="$(printf 'MARK_NODRAFT' | "$GANG" send 1 --from tester --live-only 2>&1)"; then
...
tmux send-keys -t "$(window_id 1)" C-u
tmux set-option -w -t "$(window_id 1)" @gl_turn "open v2:$(test_now_ns)"
if fresh_veto="$(printf 'MARK_FRESH' | "$GANG" send 1 --from tester --live-only 2>&1)"; then
...
tmux set-option -w -t "$(window_id vanish)" @gl_turn "open v2:1"
printf '1' > "$RUN_ROOT/vanish-tickets"
touch "$RUN_ROOT/vanish-flag"
if vanish_out="$(printf 'MARK_VANISH' | "$GANG" send vanish --from tester --live-only 2>&1)"; then
...
if emptied_out="$(printf 'MARK_EMPTIED' |
  "$GANG" send emptied --from tester --live-only 2>&1)"; then
...
if printf 'MARK_MALFORMED' | "$GANG" send 1 --from tester >/dev/null 2>&1; then
...
# An Escape-interrupted turn leaves a fossil busy
...
if printf 'MARK_ACTIVE' | GANG_ACTIVITY_WINDOW=100000 \
  "$GANG" send fossil --from tester --live-only >/dev/null 2>&1; then
...
fossil_explain_rc=0
fossil_explain="$(GANG_ACTIVITY_WINDOW=0 "$GANG" status --why fossil 2>&1)" \
  || fossil_explain_rc=$?
equal "status --why reads a quiet-at-rest agent's pty clock through its pinned pane" \
  0 "$fossil_explain_rc"
...
if printf 'MARK_FOSSIL' | GANG_ACTIVITY_WINDOW=0 \
  "$GANG" send fossil --from tester >/dev/null 2>&1; then
...
# On the path no harness reports, a turn
...
# permanent verdict while it sits provably ready. The sources under the expired
...
# fresh owned state and outranks every quiet source under it. Nothing about a
...
# its limit with the pty credited as recent: the activity check then reports
# could-not-determine, and an unknown source cannot witness readiness.
...
if printf 'MARK_DECAYED' | GANG_ACTIVITY_WINDOW=0 \
  "$GANG" send abandoned --from tester >/dev/null 2>&1; then
...
# rest, so the activity check reports inactive by abstention rather than by
...
tmux set-option -w -t "$(window_id assumed)" @gl_turn "open v2:1"
...
# Each source read is a separate moment, and a decay assembled across them can be
...
tmux set-option -w -t "$(window_id moving)" @gl_turn "open v2:1"
...
tmux set-option -w -t "$(window_id seam)" @gl_turn "open v2:1"
...
tmux set-option -w -t "$(window_id seamsend)" @gl_turn "open v2:1"
seamsend_out=""
if seamsend_out="$(printf 'MARK_LIVE_SEND' | PATH="$RUN_ROOT/bin-seam:$PATH" \
  GANG_ACTIVITY_WINDOW=0 "$GANG" send seamsend --from tester --live-only 2>&1)"; then
...
if noreader_out="$(printf 'MARK_NOREADER' | GANG_ACTIVITY_WINDOW=0 \
  "$GANG" send noreader --from tester --live-only 2>&1)"; then
...
tmux set-option -w -t "$(window_id ticker)" @gl_turn "open v2:1"
...
if ticker_out="$(printf 'MARK_TICKER' | CHURN_PANE="$CHURN_PANE" \
  ...
  "$GANG" send ticker --from tester --live-only 2>&1)"; then
...
tmux set-option -w -t "$(window_id blindbox)" @gl_turn "open v2:1"
touch "$RUN_ROOT/blindbox-flag"
if blind_out="$(printf 'MARK_BLIND' | "$GANG" send blindbox --from tester --live-only 2>&1)"; then
...
if [ "$readiness_saved_clock_is_set" = x ]; then
  export GANG_TEST_CLOCK="$readiness_saved_clock"
else
  unset GANG_TEST_CLOCK
fi
...
parking_out="$(printf 'MARK_PEER_PARKED' \
  | "$GANG" send parking --from tester --live-only 2>&1)" \
  || parking_rc=$?
...
printf -v self_command ': > %q; printf BUSY_DEFERRED; GANG_SESSION=%q GANG_COLLARS=%q %q compact --resume SELF_NEXT_\STEP; tmux wait-for -S %q; tmux wait-for %q; rm -f -- %q; tmux wait-for -S %q' \
...
equal "the deferred request binds its pane-ready delivery path" \
  "$self_request"$'\t'pane-ready \
  "$(tmux show-options -wqv -t "$self_id" @gl_self_compact_witness)"
...
equal "deferred self-compaction leaves the busy composer's reading empty" \
  "" "$("$GANG" capture --composer selfable)"
...
excludes "and its continuation does not arrive before Stop" \
  "$(pane selfable)" "SELF_NEXT_STEP"
...
if [ -n "$self_request" ]; then
  ...
  if [ -e "$self_executed" ]; then
    ...
    contains "and its requested continuation follows the command" \
      "$(pane selfable)" "SELF_NEXT_STEP"
  else
    ...
    fail "and its requested continuation follows the command" \
      "the compact command was never submitted"
  fi
...
# Requests made while the writer was broken carry no binding. The current
# pane-ready collar is the only missing-binding shape a boundary may recover.
rm -f "$self_executed"
self_legacy_out="$(TMUX_PANE="$self_tmux_pane" "$GANG" compact 2>&1)" || :
contains "a second pane-ready request is scheduled" "$self_legacy_out" \
  "self-compaction scheduled"
self_legacy_request="$(tmux show-options -wqv -t "$self_id" @gl_self_compact_requested)"
tmux set-option -uw -t "$self_id" @gl_self_compact_witness
if [ -n "$self_legacy_request" ]; then
  tmux wait-for "gang-self-compact-$self_legacy_request" &
  self_legacy_waiter=$!
  printf '%s' '{"hook_event_name":"Stop"}' |
    TMUX_PANE="$self_tmux_pane" "$GANG" hook >/dev/null
  wait "$self_legacy_waiter"
  equal "an unbound request from the broken pane-ready writer still submits" present \
    "$([ -e "$self_executed" ] && printf present || printf absent)"
  equal "and consumes the recovered request" "" \
    "$(tmux show-options -wqv -t "$self_id" @gl_self_compact_requested)"
else
  fail "an unbound request from the broken pane-ready writer still submits" \
    "the second request was not recorded"
  fail "and consumes the recovered request" \
    "the second request was not recorded"
fi
...
# discriminating: its compact command emits the exact native rejection and
# leaves an artifact if Gangline submits it. The unavailable witness must stop
# before a worker, Enter, or continuation -- while Stop is running and on
# every later boundary. (Codex itself now declares native-idle; see the block
# after this one.)
#
...
equal "a request without a witness binding is left intact" "$codex_race_token" \
  "$(tmux show-options -wqv -t "$codex_race_id" @gl_self_compact_requested)"
contains "and its missing binding is reported" \
  "$(tmux show-options -wqv -t "$codex_race_id" @gl_self_compact_failed)" \
  "has no self-compaction witness binding"
equal "an unbound request submits no native command" absent \
  "$([ -e "$codex_race_executed" ] && printf present || printf absent)"
...
equal "a mismatched stored verdict fails closed with the request intact" \
  "$codex_race_token" \
  "$(tmux show-options -wqv -t "$codex_race_id" @gl_self_compact_requested)"
contains "the mismatched verdict is explicit" \
  "$(tmux show-options -wqv -t "$codex_race_id" @gl_self_compact_failed)" \
...
equal "a refusal record preserves an operator draft" \
  "OPERATOR_DRAFT" "$("$GANG" capture --composer codex-stop-race)"
...
printf 'MARK_MAIL_PAST_FAILED_COMPACT' |
  "$GANG" send codex-stop-race --from tester >/dev/null
...
contains "queued delivery leaves the refusal on record" \
  "$(tmux show-options -wqv -t "$codex_race_id" @gl_self_compact_failed)" \
...
# THE HARNESS'S OWN RECORD IS THE POST-STOP WITNESS. Codex runs Stop inside
# its task and paints an idle composer that drops the Enter; it appends the
# turn's terminal record to its rollout only after the hook returns. A collar
# that declares GANG_SELF_COMPACT_WITNESS=native-idle reads that record through
# collar_native_idle, and the dispatcher submits nothing until it answers 0.
# The fixture stands in for Codex with a rollout file the suite appends to:
# Stop fires while the turn's newest record is still its start, the worker
# waits, the record lands, and only then does the compact command execute
# followed by the continuation.
native_idle_helper="$ROOT/collars/plugins/codex-native-idle.py"
native_idle_fixtures="$RUN_ROOT/native-idle"
mkdir -p "$native_idle_fixtures"
native_idle_read() { # $1 fixture name, $2 turn id or empty -> "rc<TAB>message"
  local out rc=0
  out="$(python3 "$native_idle_helper" "$native_idle_fixtures/$1" ${2:+"$2"})" || rc=$?
  printf '%s\t%s' "$rc" "$out"
}
native_idle_record() { # $1 lifecycle kind, $2 turn id -> one rollout line
  printf '{"type":"event_msg","payload":{"type":"%s","turn_id":"%s"}}\n' "$1" "$2"
}
native_idle_record task_started T > "$native_idle_fixtures/open.jsonl"
equal "an open turn is not idle" $'1\tturn T is still open: task_started is its newest record' \
  "$(native_idle_read open.jsonl T)"
{ native_idle_record task_started T; native_idle_record task_complete T; } \
  > "$native_idle_fixtures/done.jsonl"
equal "a persisted task_complete for the named turn is idle" $'0\ttask_complete for turn T' \
  "$(native_idle_read done.jsonl T)"
printf '%s' '{"type":"event_msg","pay' >> "$native_idle_fixtures/done.jsonl"
equal "a partial trailing line while the recorder appends does not hide the record" \
  $'0\ttask_complete for turn T' "$(native_idle_read done.jsonl T)"
{ native_idle_record task_started T; native_idle_record task_complete T
...
equal "an unnamed turn_aborted ends the turn it interrupted" $'0\tturn_aborted for turn T' \
  "$(native_idle_read aborted.jsonl T)"
{ native_idle_record task_started T; native_idle_record task_complete T; } \
  > "$native_idle_fixtures/other.jsonl"
equal "a record for another turn is no answer for the named one" \
  $'2\tthe rollout holds no lifecycle record for turn V' "$(native_idle_read other.jsonl V)"
: > "$native_idle_fixtures/empty.jsonl"
equal "an empty rollout gives no answer" $'2\tthe rollout holds no turn lifecycle record' \
  "$(native_idle_read empty.jsonl)"
equal "a missing rollout gives no answer" 2 \
  "$(native_idle_read missing.jsonl T | cut -f1)"
{ native_idle_record task_started T
  printf '%s\n' '{"type":"event_msg","payload":{"type":"task_complete"}}'; } \
  > "$native_idle_fixtures/unnamed-complete.jsonl"
equal "a task_complete that names no turn cannot speak for a named one" \
  $'2\tthe newest task_complete record names no turn, so it cannot speak for turn T' \
  "$(native_idle_read unnamed-complete.jsonl T)"
{ native_idle_record task_started T
  printf '%s\n' '{"type":"event_msg","payload":{"type":"task_complete","turn_id":7}}'; } \
  > "$native_idle_fixtures/malformed-complete.jsonl"
equal "a turn id that is not a string is no answer" \
  $'2\tthe newest task_complete record carries a turn id that is not a string' \
  "$(native_idle_read malformed-complete.jsonl T)"

native_idle_executed="$RUN_ROOT/native-idle-executed"
native_idle_busy="$RUN_ROOT/native-idle-busy"
...
# shellcheck disable=SC2034
. "$ROOT/collars/bash.sh"
GANG_COMPACT_CMD="printf NATIVE_IDLE_COMPACT; : > $native_idle_executed"
GANG_SELF_COMPACT=deferred
...
SH
  ...
  cat >> "$RUN_ROOT/collars/codex-native-idle.sh" <<SH
# A NOT-YET ANSWER IS HELD UNTIL THE SUITE RELEASES IT. The suite's sleep is
# instant, so without this hold the worker would spend its whole budget before
# the record could be appended; the hold signals that the worker is inside its
# poll, which is the moment the suite inspects. The marker file turns the hold
# off for the boundary that must run out its budget. A second, one-shot marker
# holds an IDLE answer instead, so the suite can open the next turn between
# the witness and the keystroke.
collar_native_idle() {
  local turn rc=0 out
  turn="\$(printf '%s' "\$2" | python3 -c 'import json,sys; print(json.load(sys.stdin).get("turn_id",""))' 2>/dev/null)" || turn=""
  out="\$(python3 "$native_idle_helper" "$native_idle_rollout" \${turn:+"\$turn"})" || rc=\$?
  printf '%s' "\$out"
  if [ "\$rc" -eq 1 ] && [ ! -e "$native_idle_nohold" ]; then
    tmux wait-for -S "native-idle-asked-$$"
    tmux wait-for "native-idle-poll-$$"
  elif [ "\$rc" -eq 0 ] && [ -e "$native_idle_hold_ok" ]; then
    rm -f -- "$native_idle_hold_ok"
    tmux wait-for -S "native-idle-ok-$$"
    tmux wait-for "native-idle-go-$$"
  fi
  return "\$rc"
}
SH
...
native_idle_peer_out="$(printf 'MARK_NATIVE_IDLE_PEER' |
  "$GANG" send native-idle --from tester 2>&1)" || :
...
native_idle_auto_out="$(printf 'MARK_NATIVE_IDLE_AUTO' |
  "$GANG" send native-idle --from tester 2>&1)" || :
contains "a message sent during the native turn waits for its Stop" \
...
if native_idle_stale_out="$(printf 'MARK_NATIVE_IDLE_STALE' |
  "$GANG" send native-idle --from tester 2>&1)"; then
...
if native_idle_rebind_out="$(printf 'MARK_NATIVE_IDLE_REBIND' |
  "$GANG" send native-idle --from tester --live-only 2>&1)"; then
...
printf -v native_idle_command ': > %q; printf BUSY_NATIVE_IDLE; GANG_SESSION=%q GANG_COLLARS=%q %q compact --resume NEXT_STEP_\\MARK; printf " NATIVE_IDLE_RC_%%s" "$?"; tmux wait-for -S %q; tmux wait-for %q; rm -f -- %q; tmux wait-for -S %q' \
  "$native_idle_busy" "$GANG_SESSION" "$GANG_COLLARS" "$GANG" \
  "$native_idle_requested" "$native_idle_release" "$native_idle_busy" \
  "$native_idle_released"
tmux send-keys -l -t "$native_idle_id" "$native_idle_command"
...
equal "the request binds the native-idle verdict to its token" \
  "$native_idle_request"$'\t'native-idle \
  "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_witness)"
equal "and records the continuation" NEXT_STEP_MARK \
  "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_resume)"
tmux wait-for -S "$native_idle_release"
tmux wait-for "$native_idle_released"
if [ -n "$native_idle_request" ]; then
  tmux wait-for "gang-self-compact-$native_idle_request" &
  native_idle_waiter=$!
  printf '%s' '{"hook_event_name":"Stop","turn_id":"turn-one","transcript_path":"'"$native_idle_rollout"'"}' |
    TMUX_PANE="$native_idle_pane" "$GANG" hook >/dev/null
  # The worker is inside its poll. Its barrier is still up, the pane is idle,
  # and the only thing missing is the harness's record; nothing may be typed.
  tmux wait-for "native-idle-asked-$$"
  equal "Stop with the turn still open submits nothing while the composer paints idle" absent \
    "$([ -e "$native_idle_executed" ] && printf present || printf absent)"
  excludes "and types no continuation" "$(pane native-idle)" "NEXT_STEP_MARK"
  equal "the worker is still dispatching" "$native_idle_request" \
    "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_dispatching)"
  native_idle_record task_complete turn-one >> "$native_idle_rollout"
  tmux wait-for -S "native-idle-poll-$$"
  wait "$native_idle_waiter"
  if [ -e "$native_idle_executed" ]; then
    pass "the persisted end of the turn releases the deferred compaction"
  else
  ...
  equal "without a recorded failure" "" \
    "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_failed)"
else
  fail "a native-idle collar schedules the agent's own compaction" \
    "@gl_self_compact_requested is empty"
fi

# A RECORD THAT NEVER LANDS IS NOT AT THIS BOUNDARY, NOT NEVER. The budget
# runs out, nothing is typed, and the request stands for a later Stop that
# carries a fresh payload. The agent is told once.
rm -f "$native_idle_executed"
: > "$native_idle_nohold"
native_idle_record task_started turn-two >> "$native_idle_rollout"
native_idle_second="$(TMUX_PANE="$native_idle_pane" "$GANG" compact --resume SECOND_STEP_MARK 2>&1)" || :
contains "a second self-request is scheduled after the first completed" \
  "$native_idle_second" "self-compaction scheduled"
native_idle_request="$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_requested)"
tmux wait-for "gang-self-compact-$native_idle_request" &
native_idle_waiter=$!
...
wait "$native_idle_waiter"
equal "a turn whose end is never persisted submits no compact command" absent \
  "$([ -e "$native_idle_executed" ] && printf present || printf absent)"
excludes "and no continuation" "$(pane native-idle)" "SECOND_STEP_MARK"
equal "the request stands for a later boundary" "$native_idle_request" \
  "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_requested)"
contains "and the record names the missing witness" \
  "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_failed)" \
  "has not persisted the end of this turn"
contains "the agent is told the compaction was put back" \
  "$("$GANG" queue native-idle)" "put back rather than dropped"
...
equal "the refused retry leaves the request standing" "$native_idle_request" \
  "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_requested)"
# A third native boundary spends the final attempt deterministically. A tick
# may begin after this window in its rotating cursor, so it is not evidence
# that this request was actually visited.
boundary_arm "gang-self-compact-$native_idle_request" "gang-spool-drain-$native_idle_id"
printf '%s' '{"hook_event_name":"Stop","turn_id":"turn-two","transcript_path":"'"$native_idle_rollout"'"}' |
  GANG_BOOT_TIMEOUT=2 TMUX_PANE="$native_idle_pane" "$GANG" hook >/dev/null
boundary_taken "gang-self-compact-$native_idle_request" "gang-spool-drain-$native_idle_id"
equal "the third refusal is another compaction attempt" compaction \
  "$BOUNDARY_TAKEN"
equal "the third refused dispatch drops the request" "" \
  "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_requested)"
contains "the retry limit remains visible on the window" \
  "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_failed)" \
  "three-attempt limit"
# Once the terminal record lands, a fresh request can run normally. The
# terminal refusal's boundary was free to drain its note after dropping the
# old request, so no assertion treats that note as still queued.
native_idle_record task_complete turn-two >> "$native_idle_rollout"
native_idle_rescheduled="$(TMUX_PANE="$native_idle_pane" "$GANG" compact --resume SECOND_STEP_MARK 2>&1)" || :
contains "a dropped request can be scheduled afresh" \
  "$native_idle_rescheduled" "self-compaction scheduled"
native_idle_request="$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_requested)"
boundary_arm "gang-self-compact-$native_idle_request" "gang-spool-drain-$native_idle_id"
printf '%s' '{"hook_event_name":"Stop","turn_id":"turn-two","transcript_path":"'"$native_idle_rollout"'"}' |
  TMUX_PANE="$native_idle_pane" "$GANG" hook >/dev/null
...
excludes "PostCompact leaves no older note waiting" \
  "$("$GANG" roster | grep '^native-idle ' || :)" "spooled="
...
tmux wait-for "gang-self-compact-$native_idle_request" &
native_idle_waiter=$!
printf '%s' '{"hook_event_name":"Stop","turn_id":"turn-unknown","transcript_path":"'"$native_idle_rollout"'"}' |
  TMUX_PANE="$native_idle_pane" "$GANG" hook >/dev/null
wait "$native_idle_waiter"
equal "an unanswerable turn keeps the request" "$native_idle_request" \
  "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_requested)"
equal "and submits nothing" absent \
  "$([ -e "$native_idle_executed" ] && printf present || printf absent)"
...
# continuation — and a witness read before that has certified nothing about
# the moment of the keystroke. So the witness is read once more under the
# pane lock, and a rollout that now names a later turn refuses the boundary
# with nothing typed. The hold below parks the worker between its first idle
# answer and that second read; the suite opens the next turn in the gap.
rm -f "$native_idle_executed"
native_idle_third="$(TMUX_PANE="$native_idle_pane" "$GANG" compact --resume THIRD_STEP_MARK 2>&1)" || :
contains "a third self-request is scheduled" "$native_idle_third" "self-compaction scheduled"
native_idle_request="$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_requested)"
: > "$native_idle_hold_ok"
tmux wait-for "gang-self-compact-$native_idle_request" &
native_idle_waiter=$!
printf '%s' '{"hook_event_name":"Stop","turn_id":"turn-two","transcript_path":"'"$native_idle_rollout"'"}' |
  TMUX_PANE="$native_idle_pane" "$GANG" hook >/dev/null
tmux wait-for "native-idle-ok-$$"
native_idle_record task_started turn-three >> "$native_idle_rollout"
tmux wait-for -S "native-idle-go-$$"
wait "$native_idle_waiter"
equal "a turn that opens between the witness and the keystroke submits nothing" absent \
  "$([ -e "$native_idle_executed" ] && printf present || printf absent)"
excludes "and no continuation" "$(pane native-idle)" "THIRD_STEP_MARK"
equal "the request stands" "$native_idle_request" \
  "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_requested)"
contains "and the record names the turn that opened" \
  "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_failed)" \
  "a later turn (turn-three) has started"
equal "the lock is released with the refusal" absent \
...
# The next refusal is the third for this request, so it reaches the cap before
# any later scenario can treat it as standing.
boundary_arm "gang-self-compact-$native_idle_request" "gang-spool-drain-$native_idle_id"
printf '%s' '{"hook_event_name":"Stop","turn_id":"turn-three","transcript_path":"'"$native_idle_rollout"'"}' |
  GANG_BOOT_TIMEOUT=2 TMUX_PANE="$native_idle_pane" "$GANG" hook >/dev/null
boundary_taken "gang-self-compact-$native_idle_request" "gang-spool-drain-$native_idle_id"
equal "the third later-turn refusal is another compaction attempt" compaction \
  "$BOUNDARY_TAKEN"
equal "and drops that request at its retry limit" "" \
  "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_requested)"
contains "with the terminal limit visible" \
  "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_failed)" \
  "three-attempt limit"

# A FRESH REQUEST BOUND TO THE WITNESS KEEPS ITS BINDING. The collar is
# rewritten underneath it to declare only `unavailable`, which retires a new
# request at dispatch but says nothing about one scheduled on the promise of a
# read: without the reader that read cannot happen, and the boundary refuses
# before the pane is consulted, leaving the request for a collar that can
# honour it.
native_idle_record task_complete turn-three >> "$native_idle_rollout"
native_idle_fresh="$(TMUX_PANE="$native_idle_pane" "$GANG" compact --resume THIRD_STEP_MARK 2>&1)" || :
contains "a request after the retry cap schedules afresh" \
  "$native_idle_fresh" "self-compaction scheduled"
native_idle_request="$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_requested)"
native_idle_collar unavailable
boundary_arm "gang-self-compact-$native_idle_request" "gang-spool-drain-$native_idle_id"
printf '%s' '{"hook_event_name":"Stop","turn_id":"turn-three","transcript_path":"'"$native_idle_rollout"'"}' |
...
equal "the request still stands" "$native_idle_request" \
  "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_requested)"
contains "and the record names the missing reader" \
  "$(tmux show-options -wqv -t "$native_idle_id" @gl_self_compact_failed)" \
...
# activity checks would otherwise recreate the permanent wedge this fix removes.
tmux set-option -w -t "$native_idle_id" @gl_native_idle_boundary \
  "$native_idle_peer_payload"
...
# scheduled against a witness nothing will ever read.
cat > "$RUN_ROOT/collars/native-idle-readerless.sh" <<SH
# shellcheck shell=bash
# shellcheck disable=SC2034
. "$ROOT/collars/bash.sh"
GANG_COMPACT_CMD="printf READERLESS"
GANG_SELF_COMPACT=deferred
GANG_SELF_COMPACT_WITNESS=native-idle
GANG_STOP_HOOK=1
SH
native_idle_readerless="$("$HITCH" native-idle-readerless -c native-idle-readerless -d /tmp 2>&1)" || :
contains "a native-idle declaration without collar_native_idle is refused" \
  "$native_idle_readerless" "defines no collar_native_idle"
"$GANG" drop native-idle-readerless >/dev/null 2>&1 || :
...
tmux set-option -w -t "$self_unknown_id" @gl_self_compact_requested \
  "$self_unknown_token"
tmux set-option -w -t "$self_unknown_id" @gl_self_compact_witness \
  "$self_unknown_token"$'\t'native-idle
...
self_unknown_mail="$("$GANG" queue self-unknown)"
...
tmux set-option -w -t "$compact_race_id" @gl_self_compact_witness \
  "$compact_race_token"$'\t'native-idle
: > "$compact_race_arm"
: > "$compact_race_claims"
...
tmux set-option -w -t "$compact_race_id" @gl_self_compact_witness \
  "$compact_claim_bad_token"$'\t'native-idle
...
tmux set-option -w -t "$compact_race_id" @gl_self_compact_requested \
  "$compact_delay_token"
tmux set-option -w -t "$compact_race_id" @gl_self_compact_witness \
  "$compact_delay_token"$'\t'native-idle
...
tmux set-option -w -t "$compact_race_id" @gl_self_compact_requested \
  "$compact_zero_token"
tmux set-option -w -t "$compact_race_id" @gl_self_compact_witness \
  "$compact_zero_token"$'\t'native-idle
...
tmux set-option -uw -t "$compact_race_id" @gl_self_compact_witness
tmux set-option -uw -t "$compact_race_id" @gl_self_compact_failed
...
tmux set-option -w -t "$compact_race_id" @gl_self_compact_witness \
  "$compact_release_token"$'\t'native-idle
...
# The delivery guards inside inject refuse with status 3. The dispatcher must
# retain its request for a later boundary. A draft in the box is the ordinary
# case at a Stop, because a
...
drafted_bound_mail="$(printf 'MARK_DRAFTED_BEHIND_BINDING' |
  "$GANG" send drafted --from tester 2>&1)" || :
...
# compact an ordinary thing to try. The continuation must be written only after
# the request can be made. The state
...
hookless_mail="$(printf 'MARK_HOOKLESS_BEHIND_COMPACTION' |
  "$GANG" send hookless --from tester 2>&1)" || :
...
unknown_hookless_mail="$(printf 'MARK_HOOKLESS_UNKNOWN_COMPACTION' |
  "$GANG" send hookless-unknown --from tester 2>&1)" || :
...
blind_hookless_mail="$(printf 'MARK_HOOKLESS_BLIND_COMPACTION' |
  "$GANG" send hookless-blind --from tester 2>&1)" || :
...
blind_hookless_mail="$(printf 'MARK_HOOKLESS_BLIND_RETIRED' |
  "$GANG" send hookless-blind --from tester 2>&1)" || :
contains "a queued message behind a request that cannot run waits in the spool" \
...
# The retiring tick above failed on purpose, and every later roster and status
# in the suite would report that failure until a clean pass clears it.
...
send_to_refused() {
  printf 'MARK_REFUSED_SEND' | "$GANG" send refused --from tester --live-only
}
status_of_refused() { "$GANG" status refused; }

# A refused capture on a live agent must remain unknown rather than infer
# `occupied` from a pane that was not read.
refused_status_rc=0
...
tmux set-option -w -t "$refused_id" @gl_turn "open v2:1"
...
# a live pane. The input reader probes the healthy pane without converting
...
masking_send() { # $1 = the reads to refuse, $2 = the marker to deliver
  ...
  printf '%s' "$2" | "$GANG" send masking --from tester --live-only 2>&1
}

: > "$RUN_ROOT/masking-reads.log"
: > "$RUN_ROOT/masking-count"
rm -f -- "$RUN_ROOT/masking-refuse-at"
printf 'MARK_MASKING_LOCATE' | "$GANG" send masking --from tester >/dev/null
...
printf 'MARK_ACTION_WEDGED reproduce the issue' \
  | "$GANG" send wedged --from tester >"$RUN_ROOT/action-send.out" 2>&1 \
  || action_send_rc=$?
...
printf 'MARK_ACTION_WORKING reproduce the issue' \
  | "$GANG" send working --from tester >>"$RUN_ROOT/action-send.out" 2>&1 \
  || action_send_rc=$?
