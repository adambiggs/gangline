...
case "$fixture_mode" in
  ...
  retry-held|retry-release|no-wait)
    lock_self_pid
    live_identity="$("$ROOT/libexec/gang-process-identity" \
      --tick "$LOCK_SELF_PID" "$SESSION")"
    IFS=$'\t' read -r _ live_token _ _ _ _ _ live_namespace <<<"$live_identity"
    command ln -s "v1:$LOCK_SELF_PID:$live_token:$live_namespace" "$path"
...
lock_self_pid
self_identity="$("$ROOT/libexec/gang-process-identity" \
  --tick "$LOCK_SELF_PID" "$SESSION")"
IFS=$'\t' read -r _ _ _ _ _ _ _ self_namespace <<<"$self_identity"
ln -s "v1:99999999:0:$self_namespace" "$path"
...
# sandbox's pid 1 is an unrelated live process on the host. The lock record
# carries enough process identity for a host contender to distinguish the dead
# owner without stealing from it while it is live.
namespace_run() {
