...
The sender `gangline:hitch` identifies startup generated by Gangline when
the launching pane is not a registered agent. Otherwise startup names the
launching agent. A `startup` envelope with no assignment supplies context only.
