...
# start getting expensive and red at 40% says finish the task and rotate — a
...
# COMPACTION IS DECLARED, NOT INFERRED. A compacting harness queues what it is
# sent while @gl_turn stays closed. The native pair states the condition
# directly: PreCompact opens the bracket, PostCompact closes it and drains.
# PreCompact's payload names its cause in a `trigger` field.
#
...
# file. Automatic reset scheduling may reuse its last hook sample for this many
# seconds; explicit `gang limits` and `gang wait --limit` reads remain fresh.
...
collar_usage_limits() { # $1 = unused target; print label<TAB>used<TAB>reset<TAB>observed
  local output
  # This reader runs inside native hooks when auto-resume is enabled.
