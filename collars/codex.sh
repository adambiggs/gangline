...
#
# tomllib is imported only on the path that reads the config, so an explicit
# model does not acquire that optional dependency. Its decode error is a
# ValueError, so the handler names no module the import may have failed to bind.
